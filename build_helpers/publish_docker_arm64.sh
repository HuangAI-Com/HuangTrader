...
GHCR_IMAGE_NAME=ghcr.io/HuangTrader/HuangTrader
...
if [ "${GITHUB_EVENT_NAME}" = "schedule" ]; then
    ...
    # Build regular image
    docker build -t HuangTrader:${TAG_ARM} .

else
    echo "event ${GITHUB_EVENT_NAME}: building with cache"
    # Build regular image
    docker pull ${IMAGE_NAME}:${TAG_ARM}
    docker build --cache-from ${IMAGE_NAME}:${TAG_ARM} -t HuangTrader:${TAG_ARM} .
...
docker build --build-arg sourceimage=HuangTrader --build-arg sourcetag=${TAG_ARM} -t HuangTrader:${TAG_PLOT_ARM} -f docker/Dockerfile.plot .
docker build --build-arg sourceimage=HuangTrader --build-arg sourcetag=${TAG_ARM} -t HuangTrader:${TAG_FREQAI_ARM} -f docker/Dockerfile.freqai .
docker build --build-arg sourceimage=HuangTrader --build-arg sourcetag=${TAG_FREQAI_ARM} -t HuangTrader:${TAG_FREQAI_RL_ARM} -f docker/Dockerfile.freqai_rl .

# Tag image for upload and next build step
docker tag HuangTrader:$TAG_ARM ${CACHE_IMAGE}:$TAG_ARM
docker tag HuangTrader:$TAG_PLOT_ARM ${CACHE_IMAGE}:$TAG_PLOT_ARM
docker tag HuangTrader:$TAG_FREQAI_ARM ${CACHE_IMAGE}:$TAG_FREQAI_ARM
docker tag HuangTrader:$TAG_FREQAI_RL_ARM ${CACHE_IMAGE}:$TAG_FREQAI_RL_ARM

# Run backtest
docker run --rm -v $(pwd)/tests/testdata/config.tests.json:/HuangTrader/config.json:ro -v $(pwd)/tests:/tests HuangTrader:${TAG_ARM} backtesting --datadir /tests/testdata --strategy-path /tests/strategy/strats/ --strategy StrategyTestV3
