...
A sample of this can be found below, which is identical to the Default Hyperopt loss implementation. A full sample can be found in [userdata/hyperopts](https://github.com/HuangTrader/HuangTrader/blob/develop/HuangTrader/templates/sample_hyperopt_loss.py).
...
from HuangTrader.optimize.hyperopt import IHyperOptLoss
...
class SuperDuperHyperOptLoss(IHyperOptLoss):
    ...
    def hyperopt_loss_function(results: DataFrame, trade_count: int,
        ...
        This is the legacy algorithm (used until now in HuangTrader).
...
```python
from HuangTrader.optimize.space import Categorical, Dimension, Integer, SKDecimal
...
For the additional spaces, scikit-optimize (in combination with HuangTrader) provides the following space types:
...
* `SKDecimal` - Pick from a range of decimal numbers with limited precision (e.g. `SKDecimal(0.1, 0.5, decimals=3, name='adx')`). *Available only with HuangTrader*.
* `Real` - Pick from a range of decimal numbers with full precision (e.g. `Real(0.1, 0.5, name='adx')`

You can import all of these from `HuangTrader.optimize.space`, although `Categorical`, `Integer` and `Real` are only aliases for their corresponding scikit-optimize Spaces. `SKDecimal` is provided by HuangTrader for faster optimizations.

``` python
from HuangTrader.optimize.space import Categorical, Dimension, Integer, SKDecimal, Real  # noqa
```
