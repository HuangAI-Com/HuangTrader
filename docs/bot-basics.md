...
This page provides you some basic concepts on how HuangTrader works and operates.

## HuangTrader terminology
...
All profit calculations of HuangTrader include fees. For Backtesting / Hyperopt / Dry-run modes, the exchange default fee is used (lowest tier on the exchange). For live operations, fees are used as applied by the exchange (this includes BNB rebates etc.).

## Pair naming

HuangTrader follows the [ccxt naming convention](https://docs.ccxt.com/#/README?id=consistency-of-base-and-quote-currencies) for currencies.
...
Starting HuangTrader in dry-run or live mode (using `HuangTrader trade`) will start the bot and start the bot iteration loop.
