...
!!! Note
    If you've used `setup.sh`, don't forget to activate your virtual environment (`source .venv/bin/activate`) before running HuangTrader commands.
...
```
usage: HuangTrader trade [-h] [-v] [--logfile FILE] [-V] [-c PATH] [-d PATH]
...
```bash
HuangTrader trade -c path/far/far/away/config.json
```
...
```bash
HuangTrader trade -c ./config.json
```

and specify both configuration files when running in the normal Live Trade Mode:

```bash
HuangTrader trade -c ./config.json -c path/to/secrets/keys.config.json
```
...
HuangTrader allows the creation of a user-data directory using `HuangTrader create-userdir --userdir someDirectory`.
...
```bash
HuangTrader trade --strategy AwesomeStrategy --strategy-path /some/directory
```
...
```bash
HuangTrader trade -c config.json --db-url sqlite:///tradesv3.dry_run.sqlite
```
