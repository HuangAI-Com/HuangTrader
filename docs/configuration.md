...
HuangTrader has many configurable features and possibilities.
By default, these settings are configured via the configuration file (see below).

## The HuangTrader configuration file

The bot uses a set of configuration parameters during its operation that all together conform to the bot configuration. It normally reads its configuration from a file (HuangTrader configuration file).
...
If the default configuration file is not created we recommend to use `HuangTrader new-config --config user_data/config.json` to generate a basic configuration file.

The HuangTrader configuration file is to be written in JSON format.
...
Set options in the HuangTrader configuration via environment variables.
This takes priority over the corresponding value in configuration or strategy.

Environment variables must be prefixed with  `HuangTrader__` to be loaded to the HuangTrader configuration.

`__` serves as level separator, so the format used should correspond to `HuangTrader__{section}__{key}`.
As such - an environment variable defined as  `export HuangTrader__STAKE_AMOUNT=200` would result in `{stake_amount: 200}`.

A more complex example might be `export HuangTrader__EXCHANGE__KEY=<yourExchangeKey>` to keep your exchange key secret. This will move the value to the `exchange.key` section of the configuration.
...
```
HuangTrader__TELEGRAM__CHAT_ID=<telegramchatid>
HuangTrader__TELEGRAM__TOKEN=<telegramToken>
HuangTrader__EXCHANGE__KEY=<yourExchangeKey>
HuangTrader__EXCHANGE__SECRET=<yourExchangeSecret>
```
...
!!! Tip "Use multiple configuration files to keep secrets secret"
    ...
    ``` bash
    HuangTrader trade --config user_data/config1.json --config user_data/config-private.json <...>
    ```
    ...
    ``` bash
    HuangTrader trade --config user_data/config.json <...>
    ```
...
HuangTrader can also load many options via command line (CLI) arguments (check out the commands `--help` output for details).
...
| `fee` | Fee used during backtesting / dry-runs. Should normally not be configured, which has HuangTrader fall back to the exchange default fee. Set as ratio (e.g. 0.001 = 0.1%). Fee is applied twice for each trade, once when buying, once when selling. <br> **Datatype:** Float (as ratio)
| `futures_funding_rate` | User-specified funding rate to be used when historical funding rates are not available from the exchange. This does not overwrite real historical rates. It is recommended that this be set to 0 unless you are testing a specific coin and you understand how the funding rate will affect HuangTrader's profit calculations. [More information here](leverage.md#unavailable-funding-rates) <br>*Defaults to `None`.*<br> **Datatype:** Float
...
| `experimental.block_bad_exchanges` | Block exchanges known to not work with HuangTrader. Leave on default unless you want to test if that exchange works now. <br>*Defaults to `true`.* <br> **Datatype:** Boolean
| | **Plugins**
...
| `bot_name` | Name of the bot. Passed via API to a client - can be shown to distinguish / name bots.<br> *Defaults to `HuangTrader`*<br> **Datatype:** String
...
To guarantee safe execution, HuangTrader will not allow buying with a stake-amount of 10.1$, instead, it'll make sure that there's enough space to place a stoploss below the pair (+ an offset, defined by `amount_reserve_percent`, which defaults to 5%).
...
!!! Warning
    Since the limits on exchanges are usually stable and are not updated often, some pairs can show pretty high minimum limits, simply because the price increased a lot since the last limit adjustment by the exchange. HuangTrader adjusts the stake-amount to this value, unless it's > 30% more than the calculated/desired stake-amount - in which case the trade is rejected.

#### Tradable balance

By default, the bot assumes that the `complete amount - 1%` is at it's disposal, and when using [dynamic stake amount](#dynamic-stake-amount), it will split the complete balance into `max_open_trades` buckets per trade.
HuangTrader will reserve 1% for eventual fees when entering a trade and will therefore not touch that by default.
...
For security reasons, it's disabled by default, and HuangTrader will show a warning message on startup if enabled.
For example, you can send `/forceenter ETH/BTC` to the bot, which will result in HuangTrader buying the pair and holds it until a regular exit-signal (ROI, stoploss, /forceexit) appears.
...
HuangTrader uses the Coingecko API to convert the coin value to it's corresponding fiat value for the Telegram reports.
...
HuangTrader supports both Demo and Pro coingecko API keys.
...
You will need to create API Keys (usually you get `key` and `secret`, some exchanges require an additional `password`) from the Exchange website and you'll need to insert this into the appropriate fields in the configuration or when asked by the `HuangTrader new-config` command.
...
!!! Hint "Keep your secrets secret"
    ...
    You can then start the bot with `HuangTrader trade --config user_data/config.json --config user_data/config-private.json <...>` to have your keys loaded.

    **NEVER** share your private configuration file or your exchange keys with anyone!

### Using proxy with HuangTrader

To use a proxy with HuangTrader, export your proxy settings using the variables `"HTTP_PROXY"` and `"HTTPS_PROXY"` set to the appropriate values.
