...
To download data (candles / OHLCV) needed for backtesting and hyperoptimization use the `HuangTrader download-data` command.

If no additional parameter is specified, HuangTrader will download data for `"1m"` and `"5m"` timeframes for the last 30 days.
...
!!! Tip "Tip: Updating existing data"
    If you already have backtesting data available in your data-directory and would like to refresh this data up to today, HuangTrader will automatically calculate the data missing for the existing pairs and the download will occur from the latest available point until "now", neither --days or --timerange parameters are required. HuangTrader will keep the available data and only download the missing data.
    ...
    If you use `--days xx` parameter alone - data for specified number of days will be downloaded for _all_ pairs. Be careful, if specified number of days is smaller than gap between now and last downloaded candle - HuangTrader will delete all existing data to avoid gaps in candle data.

### Usage

```
usage: HuangTrader download-data [-h] [-v] [--logfile FILE] [-V] [-c PATH]
                               [-d PATH] [--userdir PATH]
...
!!! Tip "Downloading all data for one quote currency"
    ...
    `HuangTrader download-data --exchange binance --pairs .*/USDT <...>`. The provided "pairs" string will be expanded to contain all active pairs on the exchange.
...
```bash
HuangTrader download-data --exchange binance
```
...
```bash
HuangTrader download-data --exchange binance --pairs ETH/USDT XRP/USDT BTC/USDT
```

or as regex (in this case, to download all active USDT pairs)

```bash
HuangTrader download-data --exchange binance --pairs .*/USDT
```
...
``` bash
HuangTrader download-data --exchange binance --pairs ETH/USDT XRP/USDT BTC/USDT --prepend --timerange 20210101-20220101
```

!!! Note
    HuangTrader will ignore the end-date in this mode if data is available, updating the end-date to the existing data start point.

### Data format

HuangTrader currently supports the following data-formats:
...
``` bash
time HuangTrader list-data --show-timerange --data-format-ohlcv <dataformat>
```
...
```
usage: HuangTrader convert-data [-h] [-v] [--logfile FILE] [-V] [-c PATH]
                              [-d PATH] [--userdir PATH]
...
The following command will convert all candle (OHLCV) data available in `~/.HuangTrader/data/binance` from json to jsongz, saving diskspace in the process.
It'll also remove original json data files (`--erase` parameter).

``` bash
HuangTrader convert-data --format-from json --format-to jsongz --datadir ~/.HuangTrader/data/binance -t 5m 15m --erase
```

## Sub-command convert trade data

```
usage: HuangTrader convert-trade-data [-h] [-v] [--logfile FILE] [-V] [-c PATH]
                                    [-d PATH] [--userdir PATH]
                                    [-p PAIRS [PAIRS ...]] --format-from
...
The following command will convert all available trade-data in `~/.HuangTrader/data/kraken` from jsongz to json.
It'll also remove original jsongz data files (`--erase` parameter).

``` bash
HuangTrader convert-trade-data --format-from jsongz --format-to json --datadir ~/.HuangTrader/data/kraken --erase
```
...
```
usage: HuangTrader trades-to-ohlcv [-h] [-v] [--logfile FILE] [-V] [-c PATH]
                                 [-d PATH] [--userdir PATH]
                                 [-p PAIRS [PAIRS ...]]
                                 [-t TIMEFRAMES [TIMEFRAMES ...]]
                                 [--exchange EXCHANGE]
...
``` bash
HuangTrader trades-to-ohlcv --exchange kraken -t 5m 1h 1d --pairs BTC/EUR ETH/EUR
```
...
```
usage: HuangTrader list-data [-h] [-v] [--logfile FILE] [-V] [-c PATH] [-d PATH]
...
```bash
> HuangTrader list-data --userdir ~/.HuangTrader/user_data/
...
```bash
HuangTrader download-data --exchange kraken --pairs XRP/EUR ETH/EUR --days 20 --dl-trades
```
