...
This page is intended for developers of HuangTrader, people who want to contribute to the HuangTrader codebase or documentation, or people who want to understand the source code of the application they're running.

All contributions, bug reports, bug fixes, documentation improvements, enhancements and ideas are welcome. We [track issues](https://github.com/HuangTrader/HuangTrader/issues) on [GitHub](https://github.com) and also have a dev channel on [discord](https://discord.gg/p7nuUNVfP7) where you can ask questions.

## Documentation

Documentation is available at [https://HuangTrader.io](https://www.HuangTrader.io/) and needs to be provided with every new feature PR.
...
Before opening a pull request, please familiarize yourself with our [Contributing Guidelines](https://github.com/HuangTrader/HuangTrader/blob/develop/CONTRIBUTING.md).
...
This gives developers the ability to start the bot with all required dependencies *without* needing to install any HuangTrader specific dependencies on your local machine.
...
If necessary, the HuangTrader team can assist and give guidance with writing good tests (however please don't expect anyone to write the tests for you).
...
HuangTrader uses 2 main methods to check log content in tests, `log_has()` and `log_has_re()` (to check using regex, in case of dynamic log-messages).
...
To debug HuangTrader, we recommend VSCode (with the Python extension) with the following launch configuration (located in `.vscode/launch.json`).
Details will obviously vary between setups - but this should work to get you started.

``` json
{
    "name": "HuangTrader trade",
    "type": "debugpy",
    "request": "launch",
    "module": "HuangTrader",
...
A similar setup can also be taken for Pycharm - using `HuangTrader` as module name, and setting the command line arguments as "parameters".
...
HuangTrader Exceptions all inherit from `HuangTraderException`.
...
First of all, have a look at the [VolumePairList](https://github.com/HuangTrader/HuangTrader/blob/develop/HuangTrader/pairlist/VolumePairList.py) Handler, and best copy this file with a name of your new Pairlist Handler.
...
!!! Note
    This section is a Work in Progress and is not a complete guide on how to test a new exchange with HuangTrader.
...
Also try to use `HuangTrader download-data` for an extended timerange (multiple months) and verify that the data downloaded correctly (no holes, the specified timerange was actually downloaded).
...
from HuangTrader.data.converter import ohlcv_to_dataframe
...
file = Path('HuangTrader/exchange/binance_leverage_tiers.json')
...
``` bash
jupyter nbconvert --ClearOutputPreprocessor.enabled=True --inplace HuangTrader/templates/strategy_analysis_example.ipynb
jupyter nbconvert --ClearOutputPreprocessor.enabled=True --to markdown HuangTrader/templates/strategy_analysis_example.ipynb --stdout > docs/strategy_analysis_example.md
```
...
* Docker images contain a file, `/HuangTrader/HuangTrader_commit` containing the commit this image is based of.
...
* Edit `HuangTrader/__init__.py` and add the version matching the current date (for example `2019.7` for July 2019). Minor versions can be `2019.7.1` should we need to do a second release that month. Version numbers must follow allowed versions from PEP0440 to avoid failures pushing to pypi.
* Commit this part.
