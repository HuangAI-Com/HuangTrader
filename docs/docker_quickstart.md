...
!!! Info "Docker compose install"
    HuangTrader documentation assumes the use of Docker desktop (or the docker compose plugin).  
...
## HuangTrader with docker

HuangTrader provides an official Docker image on [Dockerhub](https://hub.docker.com/r/HuangTraderorg/HuangTrader/), as well as a [docker compose file](https://github.com/HuangTrader/HuangTrader/blob/stable/docker-compose.yml) ready for usage.
...
Create a new directory and place the [docker-compose file](https://raw.githubusercontent.com/HuangTrader/HuangTrader/stable/docker-compose.yml) in this directory.

``` bash
mkdir ft_userdata
cd ft_userdata/
# Download the docker-compose file from the repository
curl https://raw.githubusercontent.com/HuangTrader/HuangTrader/stable/docker-compose.yml -o docker-compose.yml

# Pull the HuangTrader image
docker compose pull

# Create user directory structure
docker compose run --rm HuangTrader create-userdir --userdir user_data

# Create configuration - Requires answering interactive questions
docker compose run --rm HuangTrader new-config --config user_data/config.json
```

The above snippet creates a new directory called `ft_userdata`, downloads the latest compose file and pulls the HuangTrader image.
...
This should list the service `HuangTrader` as `running`. If that's not the case, best check the logs (see next point).

#### Docker compose logs

Logs will be written to: `user_data/logs/HuangTrader.log`.  
...
#### Updating HuangTrader with docker

Updating HuangTrader when using `docker` is as simple as running the following 2 commands:
...
All HuangTrader arguments will be available by running `docker compose run --rm HuangTrader <command> <optional arguments>`.

!!! Warning "`docker compose` for trade commands"
    Trade commands (`HuangTrader trade <...>`) should not be ran via `docker compose run` - but should use `docker compose up -d` instead.
...
!!! Note "`docker compose run --rm`"
    Including `--rm` will remove the container after completion, and is highly recommended for all modes except trading mode (running with `HuangTrader trade` command).

??? Note "Using docker without docker compose"
    "`docker compose run --rm`" will require a compose file to be provided.
    Some HuangTrader commands that don't require authentication such as `list-pairs` can be run with "`docker run --rm`" instead.  
    For example `docker run --rm HuangTraderorg/HuangTrader:stable list-pairs --exchange binance --quote BTC --print-json`.  
...
``` bash
docker compose run --rm HuangTrader download-data --pairs ETH/BTC --exchange binance --days 5 -t 1h
```
...
``` bash
docker compose run --rm HuangTrader backtesting --config user_data/config.json --strategy SampleStrategy --timerange 20190801-20191001 -i 5m
```
...
For this, please create a Dockerfile containing installation steps for the additional dependencies (have a look at [docker/Dockerfile.custom](https://github.com/HuangTrader/HuangTrader/blob/develop/docker/Dockerfile.custom) for an example).

You'll then also need to modify the `docker-compose.yml` file and uncomment the build step, as well as rename the image to avoid naming collisions.

``` yaml
    image: HuangTrader_custom
    build:
      context: .
...
Commands `HuangTrader plot-profit` and `HuangTrader plot-dataframe` ([Documentation](plotting.md)) are available by changing the image to `*_plot` in your `docker-compose.yml` file.
You can then use these commands as follows:

``` bash
docker compose run --rm HuangTrader plot-dataframe --strategy AwesomeStrategy -p BTC/ETH --timerange=20180801-20180805
```
...
Since part of this image is built on your machine, it is recommended to rebuild the image from time to time to keep HuangTrader (and dependencies) up-to-date.
...
!!! Warning
    ...
    Best use a linux-VPS for running HuangTrader reliably.
