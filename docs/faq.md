...
HuangTrader supports spot trading, as well as (isolated) futures trading for some selected exchanges. Please refer to the [documentation start page](index.md#supported-futures-exchanges-experimental) for an up-to-date list of supported exchanges.

### Can my bot open short positions?

HuangTrader can open short positions in futures markets.
...
In spot markets, you can in some cases use leveraged spot tokens, which reflect an inverted pair (eg. BTCUP/USD, BTCDOWN/USD, ETHBULL/USD, ETHBEAR/USD,...) which can be traded with HuangTrader.
...
* When you work with your strategy & hyperopt file you should use a proper code editor like VSCode or PyCharm. A good code editor will provide syntax highlighting as well as line numbers, making it easy to find syntax errors (most likely pointed out by HuangTrader during startup).

## HuangTrader common questions

### Can HuangTrader open multiple positions on the same pair in parallel?

No. HuangTrader will only open one position per pair at a time.
...
Running the bot with `HuangTrader trade --config config.json` shows the output `HuangTrader: command not found`.
...
While HuangTrader can handle this (it'll sell 99 COIN), fees are often below the minimum tradable lot-size (you can only trade full COIN, not 0.9 COIN).
Leaving the dust (0.9 COIN) on the exchange makes usually sense, as the next time HuangTrader buys COIN, it'll eat into the remaining small balance, this time selling everything it bought, and therefore slowly declining the dust balance (although it most likely will never reach exactly 0).

Where possible (e.g. on binance), the use of the exchange's dedicated fee currency will fix this.
On binance, it's sufficient to have BNB in your account, and have "Pay fees in BNB" enabled in your profile. Your BNB balance will slowly decline (as it's used to pay fees) - but you'll no longer encounter dust (HuangTrader will include the fees in the profit calculations).
...
HuangTrader will update the exchange balance when necessary (Before placing an order).
...
HuangTrader will not provide incomplete candles to strategies. Using incomplete candles will lead to repainting and consequently to strategies with "ghost" buys, which are impossible to both backtest, and verify after they happened.
...
Please look at the [advanced setup documentation Page](advanced-setup.md#running-multiple-instances-of-HuangTrader).
...
Irrespectively of the reason, HuangTrader will fill up these candles with "empty" candles, where open, high, low and close are set to the previous candle close - and volume is empty. In a chart, this will look like a `_` - and is aligned with how exchanges usually represent 0 volume candles.
...
As a consequence, HuangTrader will not enter a trade for this pair - as trading on old information is usually not what is desired.
...
By default, the bot writes its log into stderr stream. This is implemented this way so that you can easily separate the bot's diagnostics messages from Backtesting, Edge and Hyperopt results, output from other various HuangTrader utility sub-commands, as well as from the output of your custom `print()`'s you may have inserted into your strategy. So if you need to search the log messages with the grep utility, you need to redirect stderr to stdout and disregard stdout.

* In unix shells, this normally can be done as simple as:
```shell
$ HuangTrader --some-options 2>&1 >/dev/null | grep 'something'
```
...
```shell
$ HuangTrader --some-options 2> >(grep 'something') >/dev/null
```
or
```shell
$ HuangTrader --some-options 2> >(grep -v 'something' 1>&2)
```

* You can also write the copy of HuangTrader log messages to a file with the `--logfile` option:
```shell
$ HuangTrader --logfile /path/to/mylogfile.log --some-options
```
and then grep it as:
```shell
...
On Windows, the `--logfile` option is also supported by HuangTrader and you can use the `findstr` command to search the log for the string of interest:
```
...
### Why does HuangTrader not have GPU support?

First of all, most indicator libraries don't have GPU support - as such, there would be little benefit for indicator calculations.
The GPU improvements would only apply to pandas-native calculations - or ones written by yourself.

For hyperopt, HuangTrader is using scikit-optimize, which is built on top of scikit-learn.
...
```bash
HuangTrader hyperopt --hyperopt-loss SharpeHyperOptLossDaily --strategy SampleStrategy -e 1000
```

### Why does it take a long time to run hyperopt?

* Discovering a great strategy with Hyperopt takes time. Study www.HuangTrader.io, the HuangTrader Documentation page, join the HuangTrader [discord community](https://discord.gg/p7nuUNVfP7). While you patiently wait for the most advanced, free crypto bot in the world, to hand you a possible golden strategy specially designed just for you.
...
Example:
`HuangTrader --config config.json --strategy SampleStrategy --hyperopt SampleHyperopt -e 1000 --timerange 20190601-20200601`

## Edge module

### Edge implements interesting approach for controlling position size, is there any theory behind it?

The Edge module is mostly a result of brainstorming of [@mishaker](https://github.com/mishaker) and [@creslinux](https://github.com/creslinux) HuangTrader team members.
...
HuangTrader is using exclusively the following official channels:

* [HuangTrader discord server](https://discord.gg/p7nuUNVfP7)
* [HuangTrader documentation (https://HuangTrader.io)](https://HuangTrader.io)
* [HuangTrader github organization](https://github.com/HuangTrader)

Nobody affiliated with the HuangTrader project will ask you about your exchange keys or anything else exposing your funds to exploitation.
...
Failing to follow these guidelines will not be responsibility of HuangTrader.

## "HuangTrader token"

HuangTrader does not have a Crypto token offering.

Token offerings you find on the internet referring HuangTrader, FreqAI or freqUI must be considered to be a scam, trying to exploit HuangTrader's popularity for their own, nefarious gains.
