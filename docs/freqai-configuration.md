...
FreqAI is configured through the typical [HuangTrader config file](configuration.md) and the standard [HuangTrader strategy](strategy-customization.md). Examples of FreqAI config and strategy files can be found in `config_examples/config_freqai.example.json` and `HuangTrader/templates/FreqaiExampleStrategy.py`, respectively.
...
The FreqAI strategy requires including the following lines of code in the standard [HuangTrader strategy](strategy-customization.md):
...
!!! Note
    ...
    (as exemplified in `HuangTrader/templates/FreqaiExampleStrategy.py`).
...
The `startup_candle_count` in the FreqAI strategy needs to be set up in the same way as in the standard HuangTrader strategy (see details [here](strategy-customization.md#strategy-startup-period)). This value is used by HuangTrader to ensure that a sufficient amount of data is provided when calling the `dataprovider`, to avoid any NaNs at the beginning of the first training. You can easily set this value by identifying the longest period (in candle units) which is passed to the indicator creation functions (e.g., TA-Lib functions). In the presented example, `startup_candle_count` is 20 since this is the maximum value in `indicators_periods_candles`.
...
!!! Note
    ...
    ```
    2022-08-31 15:14:04 - HuangTrader.freqai.data_kitchen - INFO - dropped 0 training points due to NaNs in populated dataset 4319.
    ```
...
```bash
HuangTrader trade --config config_examples/config_freqai.example.json --strategy FreqaiExampleStrategy --freqaimodel PyTorchMLPRegressor --strategy-path HuangTrader/templates 
```
...
??? Note "Setting Class Names for Classifiers"
    ...
    def set_freqai_targets(self, dataframe: DataFrame, metadata: Dict, **kwargs) -> DataFrame:
        ...
        return dataframe
    ```
    To see a full example, you can refer to the [classifier test strategy class](https://github.com/HuangTrader/HuangTrader/blob/develop/tests/strategy/strats/freqai_test_classifier.py).
