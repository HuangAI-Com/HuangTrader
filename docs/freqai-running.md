...
```bash
HuangTrader trade --strategy FreqaiExampleStrategy --config config_freqai.example.json --freqaimodel LightGBMRegressor
```
...
```bash
HuangTrader backtesting --strategy FreqaiExampleStrategy --strategy-path HuangTrader/templates --config config_examples/config_freqai.example.json --freqaimodel LightGBMRegressor --timerange 20210501-20210701
```
...
You can hyperopt using the same command as for [typical HuangTrader hyperopt](hyperopt.md):

```bash
HuangTrader hyperopt --hyperopt-loss SharpeHyperOptLoss --strategy FreqaiExampleStrategy --freqaimodel LightGBMRegressor --strategy-path HuangTrader/templates --config config_examples/config_freqai.example.json --timerange 20220428-20220507
```
...
!!! note "Availability"
    FreqAI includes tensorboard for a variety of models, including XGBoost, all PyTorch models, Reinforcement Learning, and Catboost. If you would like to see Tensorboard integrated into another model type, please open an issue on the [HuangTrader GitHub](https://github.com/HuangTrader/HuangTrader/issues)
