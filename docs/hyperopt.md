...
!!! Bug
    Hyperopt can crash when used with only 1 CPU Core as found out in [Issue #1133](https://github.com/HuangTrader/HuangTrader/issues/1133)
...
```
usage: HuangTrader hyperopt [-h] [-v] [--logfile FILE] [-V] [-c PATH] [-d PATH]
...
!!! Tip "Quickly optimize ROI, stoploss and trailing stoploss"
    ...
    HuangTrader hyperopt --hyperopt-loss SharpeHyperOptLossDaily --spaces roi stoploss trailing --strategy MyWorkingStrategy --config config.json -e 100
    ```
...
For every new set of parameters, HuangTrader will run first `populate_entry_trend()` followed by `populate_exit_trend()`, and then run the regular backtesting process to simulate trades.
...
By default, we assume a stoploss of 5% - and a take-profit (`minimal_roi`) of 10% - which means HuangTrader will sell the trade once 10% profit has been reached.
...
from HuangTrader.strategy import (BooleanParameter, CategoricalParameter, DecimalParameter, 
                                IStrategy, IntParameter)
import HuangTrader.vendor.qtpylib.indicators as qtpylib
...
HuangTrader can also optimize protections. How you optimize protections is up to you, and the following should be considered as example only.
...
`HuangTrader hyperopt --hyperopt-loss SharpeHyperOptLossDaily --strategy MyAwesomeStrategy --spaces protection`
...
!!! Note
    ...
    HuangTrader will also automatically change the "--enable-protections" flag if the protection space is selected.
...
* `ShortTradeDurHyperOptLoss` - (default legacy HuangTrader hyperoptimization loss function) - Mostly for short trade duration and avoiding losses.
...
```bash
HuangTrader hyperopt --config config.json --hyperopt-loss <hyperoptlossname> --strategy <strategyname> -e 500 --spaces all
```
...
```bash
HuangTrader hyperopt --strategy <strategyname> --timerange 20210101-20210201
```
...
If you are optimizing ROI, HuangTrader creates the 'roi' optimization hyperspace for you -- it's the hyperspace of components for the ROI tables. By default, each ROI table generated by the HuangTrader consists of 4 rows (steps). Hyperopt implements adaptive ranges for ROI tables with ranges for values in the ROI steps that depend on the timeframe used. By default the values vary in the following ranges (for some of the most used timeframes, values are rounded to 3 digits after the decimal point):
...
If you have the `generate_roi_table()` and `roi_space()` methods in your custom hyperopt, remove them in order to utilize these adaptive ROI tables and the ROI hyperoptimization space generated by HuangTrader by default.
...
If you are optimizing stoploss values, HuangTrader creates the 'stoploss' optimization hyperspace for you. By default, the stoploss values in that hyperspace vary in the range -0.35...-0.02, which is sufficient in most cases.

If you have the `stoploss_space()` method in your custom hyperopt file, remove it in order to utilize Stoploss hyperoptimization space generated by HuangTrader by default.
...
By default, hyperopt emulates the behavior of the HuangTrader Live Run/Dry Run, where only one
...
HuangTrader tries to counter the "local minima" problem by using new, randomized points in this case.
...
* You may have added parameters to hyperopt in `populate_indicators()` where they will be calculated only once **for all epochs**. If you are, for example, trying to optimise multiple SMA timeperiod values, the hyperoptable timeperiod parameter should be placed in `populate_entry_trend()` which is calculated every epoch. See [Optimizing an indicator parameter](https://www.HuangTrader.io/en/stable/hyperopt/#optimizing-an-indicator-parameter).
