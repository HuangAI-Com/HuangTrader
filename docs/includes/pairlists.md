...
It allows the user to fetch a pairlist from a remote server or a locally stored json file within the HuangTrader directory, enabling dynamic updates and customization of the trading pairlist.
...
This filter allows HuangTrader to ignore pairs until they have been listed for at least `min_days_listed` days and listed before `max_days_listed`.
