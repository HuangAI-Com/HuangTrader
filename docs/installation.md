...
The HuangTrader documentation describes various ways to install HuangTrader
...
Please consider using the prebuilt [docker images](docker_quickstart.md) to get started quickly while evaluating how HuangTrader works.
...
The easiest way to install and run HuangTrader is to clone the bot Github repository and then run the `./setup.sh` script, if it's available for your platform.
...
!!! Note
    Python3.9 or higher and the corresponding `pip` are assumed to be available. The install-script will warn you and stop if that's not the case. `git` is also needed to clone the HuangTrader repository.  
...
!!! Note "ARM64 systems"
    If you are running an ARM64 system (like a MacOS M1 or an Oracle VM), please use [docker](docker_quickstart.md) to run HuangTrader.
...
=== "RaspberryPi/Raspbian"
    ...
    This image comes with python3.9 preinstalled, making it easy to get HuangTrader up and running.
...
## HuangTrader repository

HuangTrader is an open source crypto-currency trading bot, whose code is hosted on `github.com`

```bash
# Download `develop` branch of HuangTrader repository
git clone https://github.com/HuangTrader/HuangTrader.git
...
??? Note "Install from pypi"
    An alternative way to install HuangTrader is from [pypi](https://pypi.org/project/HuangTrader/). The downside is that this method requires ta-lib to be correctly installed beforehand, and is therefore currently not the recommended way to install HuangTrader.

    ``` bash
    pip install HuangTrader
    ```
...
First of the ways to install HuangTrader, is to use provided the Linux/MacOS `./setup.sh` script, which install all dependencies and help you configure the bot.

Make sure you fulfill the [Requirements](#requirements) and have downloaded the [HuangTrader repository](#HuangTrader-repository).

### Use /setup.sh -install (Linux/MacOS)

If you are on Debian, Ubuntu or MacOS, HuangTrader provides the script to install HuangTrader.

```bash
# --install, Install HuangTrader from scratch
./setup.sh -i
```
...
You will run HuangTrader in separated `virtual environment`

```bash
# create virtualenv in directory /HuangTrader/.venv
python3 -m venv .venv
...
HuangTrader can also be installed with Miniconda or Anaconda. We recommend using Miniconda as it's installation footprint is smaller. Conda will automatically prepare and manage the extensive library-dependencies of the HuangTrader program.
...
#### HuangTrader download

Download and install HuangTrader.

```bash
# download HuangTrader
git clone https://github.com/HuangTrader/HuangTrader.git

# enter downloaded directory 'HuangTrader'
cd HuangTrader      
```

#### HuangTrader install: Conda Environment

```bash
conda create --name HuangTrader python=3.12
```
...
#### Enter/exit HuangTrader environment
...
# activate HuangTrader environment
conda activate HuangTrader
...
You've made it this far, so you have successfully installed HuangTrader.
...
HuangTrader new-config --config user_data/config.json
```
...
```bash
HuangTrader trade --config user_data/config.json --strategy SampleStrategy
```
...
```bash
# if:
bash: HuangTrader: command not found
