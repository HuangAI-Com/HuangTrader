...
You can consume the API by using `HuangTrader-client` (also available as `scripts/rest_client.py`).
This command can be installed independent of the bot by using `pip install HuangTrader-client`.

This module is designed to be lightweight, and only depends on the `requests` and `python-rapidjson` modules, skipping all heavy dependencies HuangTrader otherwise needs.

``` bash
HuangTrader-client <command> [optional parameters]
```
...
``` bash
HuangTrader-client --config rest_config.json <command> [optional parameters]
```

??? Note "Programmatic use"
    The `HuangTrader-client` package (installable independent of HuangTrader) can be used in your own scripts to interact with the HuangTrader API.
    to do so, please use the following:

    ``` python
    from HuangTrader_client import FtRestClient
...
The API Server includes a websocket endpoint for subscribing to RPC messages from the HuangTrader Bot.
...
This is also used to setup [Producer/Consumer mode](producer-consumer.md) in HuangTrader.
...
For a list of message types, please refer to the RPCMessageType enum in `HuangTrader/enums/rpcmessagetype.py`
...
Please make sure to replace `<HuangTrader_listen_ip>` (and the subsequent port) with the IP and Port matching your configuration/setup.
...
http {
    ...
    server {
        ...
        location / {
            ...
            proxy_pass http://<HuangTrader_listen_ip>:8080;
...
!!! Tip "SSL certificates"
    ...
    While this will protect your data in transit, we do not recommend to run the HuangTrader API outside of your private network (VPN, SSH tunnel).
...
!!! Note
    The below should be done in an application (a HuangTrader REST API client, which fetches info via API), and is not intended to be used on a regular basis.

HuangTrader's REST API also offers JWT (JSON Web Tokens).
You can login using the following command, and subsequently use the resulting access_token.

``` bash
> curl -X POST --user HuangTrader http://localhost:8080/api/v1/token/login
