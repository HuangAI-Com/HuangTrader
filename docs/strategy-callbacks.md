...
HuangTrader will fall back to the `proposed_stake` value should your code raise an exception. The exception itself will be logged.
...
Stoploss on exchange works similar to `trailing_stop`, and the stoploss on exchange is updated as configured in `stoploss_on_exchange_interval` ([More details about stoploss on exchange](stoploss.md#stop-loss-on-exchangeHuangTrader)).
...
For this, HuangTrader will make an additional call with `after_fill=True` after an order fills, which will allow the strategy to move the stoploss in any direction (also widening the gap between stoploss and current price, which is otherwise forbidden).
...
class AwesomeStrategy(IStrategy):
    ...
    def custom_stoploss(self, pair: str, trade: 'Trade', current_time: datetime,
        ...
        For full documentation please go to https://www.HuangTrader.io/en/latest/strategy-advanced/
...
from datetime import datetime
from HuangTrader.persistence import Trade
from HuangTrader.strategy import stoploss_from_open
...
??? Example "Returning a stoploss relative to the open price from the custom stoploss function"
    ...
    from datetime import datetime
    from HuangTrader.persistence import Trade
    from HuangTrader.strategy import IStrategy, stoploss_from_open
...
??? Example "Returning a stoploss using absolute price from the custom stoploss function"
    ...
    from datetime import datetime
    from HuangTrader.persistence import Trade
    from HuangTrader.strategy import IStrategy, stoploss_from_absolute, timeframe_to_prev_date
...
By default, HuangTrader use the orderbook to automatically set an order price([Relevant documentation](configuration.md#prices-used-for-orders)), you also have the option to create custom order prices based on your strategy.
...
However, HuangTrader also offers a custom callback for both order types, which allows you to decide based on custom criteria if an order did time out or not.
...
``` python
from datetime import datetime, timedelta
from HuangTrader.persistence import Trade, Order
...
For performance reasons, it's disabled by default and HuangTrader will show a warning message on startup if enabled.
