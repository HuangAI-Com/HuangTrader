...
We have put a great effort into keeping compatibility with existing strategies, so if you just want to continue using HuangTrader in __spot markets__, there should be no changes necessary for now.
...
class MyCoolFreqaiModel(BaseRegressionModel):
    """
    Some cool custom IFreqaiModel you made before HuangTrader version 2023.6
    """
    def train(
