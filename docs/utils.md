...
Creates the directory structure to hold your files for HuangTrader.
...
```
usage: HuangTrader create-userdir [-h] [--userdir PATH] [--reset]
...
```
usage: HuangTrader new-config [-h] [-c PATH]
...
!!! Warning
    Only vital questions are asked. HuangTrader offers a lot more configuration possibilities, which are listed in the [Configuration documentation](configuration.md#configuration-parameters)

### Create config examples

```
$ HuangTrader new-config --config user_data/config_binance.json
...
```
usage: HuangTrader show-config [-h] [--userdir PATH] [-c PATH]
                             [--show-sensitive]
...
``` output
usage: HuangTrader new-strategy [-h] [--userdir PATH] [-s NAME]
...
```bash
HuangTrader new-strategy --strategy AwesomeStrategy
```

With custom user directory

```bash
HuangTrader new-strategy --userdir ~/.HuangTrader/ --strategy AwesomeStrategy
```

Using the advanced template (populates all optional functions and methods)

```bash
HuangTrader new-strategy --strategy AwesomeStrategy --template advanced
```
...
```
usage: HuangTrader list-strategies [-h] [-v] [--logfile FILE] [-V] [-c PATH]
                                 [-d PATH] [--userdir PATH]
...
``` bash
HuangTrader list-strategies
```

Example: Search strategies  directory within the userdir.

``` bash
HuangTrader list-strategies --userdir ~/.HuangTrader/
```

Example: Search dedicated strategy path.

``` bash
HuangTrader list-strategies --strategy-path ~/.HuangTrader/strategies/
```
...
```
usage: HuangTrader list-freqaimodels [-h] [-v] [--logfile FILE] [-V] [-c PATH]
                                   [-d PATH] [--userdir PATH]
...
```
usage: HuangTrader list-exchanges [-h] [-1] [-a]
...
```
$ HuangTrader list-exchanges
Exchanges available for HuangTrader:
...
Example: see all exchanges supported by the ccxt library (including 'bad' ones, i.e. those that are known to not work with HuangTrader)

```
$ HuangTrader list-exchanges -a
...
```
usage: HuangTrader list-timeframes [-h] [-v] [--logfile FILE] [-V] [-c PATH]
                                 [-d PATH] [--userdir PATH]
                                 [--exchange EXCHANGE] [-1]
...
```
$ HuangTrader list-timeframes -c config_binance.json
...
Timeframes available for the exchange `binance`: 1m, 3m, 5m, 15m, 30m, 1h, 2h, 4h, 6h, 8h, 12h, 1d, 3d, 1w, 1M
```

* Example: enumerate exchanges available for HuangTrader and print timeframes supported by each of them:
```
$ for i in `HuangTrader list-exchanges -1`; do HuangTrader list-timeframes --exchange $i; done
```
...
For pairs traded by HuangTrader the pair quote currency is defined by the value of the `stake_currency` configuration setting.
...
```
usage: HuangTrader list-markets [-h] [-v] [--logfile FILE] [-V] [-c PATH]
                              [-d PATH] [--userdir PATH] [--exchange EXCHANGE]
                              [--print-list] [--print-json] [-1] [--print-csv]
                              [--base BASE_CURRENCY [BASE_CURRENCY ...]]
                              [--quote QUOTE_CURRENCY [QUOTE_CURRENCY ...]]
                              [-a] [--trading-mode {spot,margin,futures}]
usage: HuangTrader list-pairs [-h] [-v] [--logfile FILE] [-V] [-c PATH]
                            [-d PATH] [--userdir PATH] [--exchange EXCHANGE]
                            [--print-list] [--print-json] [-1] [--print-csv]
                            [--base BASE_CURRENCY [BASE_CURRENCY ...]]
...
```
$ HuangTrader list-pairs --quote USD --print-json
```
...
```
$ HuangTrader list-pairs -c config_binance.json --all --base BTC ETH --quote USDT USD --print-list
```

* Print all markets on exchange "Kraken", in the tabular format:

```
$ HuangTrader list-markets --exchange kraken --all
```
...
```
usage: HuangTrader test-pairlist [-h] [--userdir PATH] [-v] [-c PATH]
                               [--quote QUOTE_CURRENCY [QUOTE_CURRENCY ...]]
...
```
HuangTrader test-pairlist --config config.json --quote USDT BTC
```

## Convert database

`HuangTrader convert-db` can be used to convert your database from one system to another (sqlite -> postgres, postgres -> other postgres), migrating all trades, orders and Pairlocks.
...
```
usage: HuangTrader convert-db [-h] [--db-url PATH] [--db-url-from PATH]
...
!!! Warning
    Please ensure to only use this on an empty target database. HuangTrader will perform a regular migration, but may fail if entries already existed.
...
Run HuangTrader in webserver mode.
HuangTrader will start the webserver and allow FreqUI to start and control backtesting processes.
...
```
usage: HuangTrader webserver [-h] [-v] [--logfile FILE] [-V] [-c PATH] [-d PATH]
                           [--userdir PATH]
...
You can use `docker compose run --rm -p 127.0.0.1:8080:8080 HuangTrader webserver` to start a one-off container that'll be removed once you stop it. This assumes that port 8080 is still available and no other bot is running on that port.
...
```
usage: HuangTrader backtesting-show [-h] [-v] [--logfile FILE] [-V] [-c PATH]
                                  [-d PATH] [--userdir PATH]
...
```
usage: HuangTrader backtesting-analysis [-h] [-v] [--logfile FILE] [-V]
...
```
usage: HuangTrader hyperopt-list [-h] [-v] [--logfile FILE] [-V] [-c PATH]
...
```
HuangTrader hyperopt-list --profitable --no-details
```
...
```
usage: HuangTrader hyperopt-show [-h] [-v] [--logfile FILE] [-V] [-c PATH]
                               [-d PATH] [--userdir PATH] [--best]
...
```
HuangTrader hyperopt-show -n 168
```

Prints JSON data with details for the last best epoch (i.e., the best of all epochs):

```
HuangTrader hyperopt-show --best -n -1 --print-json --no-header
```
...
```
usage: HuangTrader show-trades [-h] [-v] [--logfile FILE] [-V] [-c PATH]
...
``` bash
HuangTrader show-trades --db-url sqlite:///tradesv3.sqlite --trade-ids 2 3 --print-json
```
...
```
usage: HuangTrader strategy-updater [-h] [-v] [--logfile FILE] [-V] [-c PATH]
                                  [-d PATH] [--userdir PATH]
