...
from HuangTrader.configuration.deprecated_settings import process_deprecated_setting
from HuangTrader.enums import RunMode, TradingMode
...
def _extend_validator(validator_class):
    """
    Extended validator for the HuangTrader configuration JSON Schema.
...
HuangTraderValidator = _extend_validator(Draft4Validator)
...
def validate_config_schema(conf: Dict[str, Any], preliminary: bool = False) -> Dict[str, Any]:
    ...
    try:
        HuangTraderValidator(conf_schema).validate(conf)
        return conf
