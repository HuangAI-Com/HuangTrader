...
from HuangTrader.constants import ENV_VAR_PREFIX
from HuangTrader.misc import deep_merge_dicts
...
def _flat_vars_to_nested_dict(env_dict: Dict[str, Any], prefix: str) -> Dict[str, Any]:
    """
    Environment variables must be prefixed with HuangTrader.
    HuangTrader__{section}__{key}
    :param env_dict: Dictionary to validate - usually os.environ
    :param prefix: Prefix to consider (usually HuangTrader__)
...
def enironment_vars_to_dict() -> Dict[str, Any]:
    ...
    Relevant variables must follow the HuangTrader__{section}__{key} pattern
    :return: Nested dict based on available and relevant variables.
    """
