...
from HuangTrader.constants import (
...
from HuangTrader.data.converter import clean_ohlcv_dataframe, ohlcv_to_dataframe, trades_dict_to_list
from HuangTrader.enums import OPTIMIZE_MODES, CandleType, MarginMode, PriceType, RunMode, TradingMode
...
from HuangTrader.exchange.common import (
...
from HuangTrader.exchange.types import OHLCVResponse, OrderBook, Ticker, Tickers
from HuangTrader.misc import (
    chunks,
    deep_merge_dicts,
    file_dump_json,
    file_load_json,
...
from HuangTrader.util import dt_from_ts, dt_now
from HuangTrader.util.datetime_helpers import dt_humanize_delta, dt_ts
...
class Exchange:
    ...
    def market_is_tradable(self, market: Dict[str, Any]) -> bool:
        """
        Check if the market symbol is tradable by HuangTrader.
    ...
    def validate_timeframes(self, timeframe: Optional[str]) -> None:
        ...
        if (
            ...
            raise ConfigurationError("Timeframes < 1m are currently not supported by HuangTrader.")
    ...
    def validate_trading_mode_and_margin_mode(
        ...
        """
        Checks if HuangTrader can perform trades using the configured
        trading mode(Margin, Futures) and MarginMode(Cross, Isolated)
        Throws OperationalException:
            If the trading_mode/margin_mode type are not supported by HuangTrader on this exchange
        """
        if trading_mode != TradingMode.SPOT and (
            ...
            raise OperationalException(
                f"HuangTrader does not support {mm_value} {trading_mode.value} on {self.name}"
            )
    ...
    def fetch_dry_run_order(self, order_id) -> Dict[str, Any]:
        ...
        try:
            ...
            from HuangTrader.persistence import Order
    ...
    def dry_run_liquidation_price(
        ...
        if self.trading_mode == TradingMode.FUTURES and self.margin_mode == MarginMode.ISOLATED:
            if market["inverse"]:
                raise OperationalException("HuangTrader does not yet support inverse contracts")
            ...
            raise OperationalException(
                "HuangTrader only supports isolated futures for leverage trading"
            )
