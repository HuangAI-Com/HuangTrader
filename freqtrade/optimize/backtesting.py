...
from HuangTrader.configuration import TimeRange, validate_config_consistency
from HuangTrader.constants import DATETIME_PRINT_FORMAT, Config, IntOrInf, LongShort
from HuangTrader.data import history
from HuangTrader.data.btanalysis import find_existing_backtest_stats, trade_list_to_dataframe
from HuangTrader.data.converter import trim_dataframe, trim_dataframes
from HuangTrader.data.dataprovider import DataProvider
from HuangTrader.data.metrics import combined_dataframes_with_rel_mean
...
from HuangTrader.optimize.backtest_caching import get_strategy_run_id
from HuangTrader.optimize.bt_progress import BTProgress
from HuangTrader.optimize.optimize_reports import (
...
from HuangTrader.persistence import (
    CustomDataWrapper,
    LocalTrade,
    Order,
    PairLocks,
...
from HuangTrader.types import BacktestResultType, get_BacktestResultType_default
from HuangTrader.util import FtPrecise
from HuangTrader.util.migrations import migrate_data
...
class Backtesting:
    ...
    def __init__(self, config: Config, exchange: Optional[Exchange] = None) -> None:
        ...
        if self.config.get("freqai_backtest_live_models", False):
            from HuangTrader.freqai.utils import get_timerange_backtest_live_models
    ...
    def _get_close_rate_for_roi(
        ...
        if roi is not None and roi_entry is not None:
            ...
            if trade_dur == 0 and (
                ...
                # details: https: // github.com/HuangTrader/HuangTrader/issues/6261
        ...
        if exit_.exit_flag:
            ...
            if exit_.exit_type in (
                ...
                if order_type == "limit":
                    ...
                    # HuangTrader does not support this in live, and the order would fill immediately
                    if trade.is_short:
    ...
    def get_valid_price_and_stake(
        ...
        if order_type == "limit":
            ...
            # which HuangTrader does not support in live.
