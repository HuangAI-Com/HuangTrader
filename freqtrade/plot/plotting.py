...
from HuangTrader.data.history import get_timerange, load_data
...
from HuangTrader.exchange import timeframe_to_prev_date, timeframe_to_seconds
from HuangTrader.misc import pair_to_filename
...
from HuangTrader.strategy import IStrategy
...
def generate_profit_graph(
    ...
    fig["layout"].update(title="HuangTrader Profit plot")
    fig["layout"]["yaxis1"].update(title="Price")
...
def generate_plot_filename(pair: str, timeframe: str) -> str:
    ...
    file_name = "HuangTrader-plot-" + pair_s + "-" + timeframe + ".html"
...
def plot_profit(config: Config) -> None:
    ...
    store_plot_file(
        fig,
        filename="HuangTrader-profit-plot.html",
        directory=config["user_data_dir"] / "plot",
