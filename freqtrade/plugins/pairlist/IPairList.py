...
from HuangTrader.exchange import Exchange, market_is_active
from HuangTrader.exchange.types import Ticker, Tickers
    ...
    def _whitelist_for_active_markets(self, pairlist: List[str]) -> List[str]:
        ...
        for pair in pairlist:
            ...
            if not self._exchange.market_is_tradable(markets[pair]):
                self.log_once(
                    f"Pair {pair} is not tradable with HuangTrader. Removing it from whitelist..",
                    logger.warning,
                )
                continue
