...
from HuangTrader.rpc.api_server.deps import get_config, get_exchange, get_rpc, get_rpc_optional
...
def show_config(rpc: Optional[RPC] = Depends(get_rpc_optional), config=Depends(get_config)):
    ...
    if rpc:
        state = rpc._HuangTrader.state
        strategy_version = rpc._HuangTrader.strategy.version()
...
def list_exchanges(config=Depends(get_config)):
    from HuangTrader.exchange import list_available_exchanges
