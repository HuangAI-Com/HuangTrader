...
from HuangTrader.constants import CANCEL_REASON, DEFAULT_DATAFRAME_COLUMNS, Config
from HuangTrader.data.history import load_data
from HuangTrader.data.metrics import DrawDownResult, calculate_expectancy, calculate_max_drawdown
...
from HuangTrader.exceptions import ExchangeError, PricingError
from HuangTrader.exchange import timeframe_to_minutes, timeframe_to_msecs
from HuangTrader.exchange.types import Tickers
from HuangTrader.loggers import bufferHandler
from HuangTrader.persistence import KeyStoreKeys, KeyValueStore, PairLocks, Trade
from HuangTrader.persistence.models import PairLock
from HuangTrader.plugins.pairlist.pairlist_helpers import expand_pairlist
from HuangTrader.rpc.fiat_convert import CryptoToFiatConverter
from HuangTrader.rpc.rpc_types import RPCSendMsg
from HuangTrader.util import decimals_per_coin, dt_now, dt_ts_def, format_date, shorten_date
from HuangTrader.util.datetime_helpers import dt_humanize_delta
from HuangTrader.wallets import PositionWallet, Wallet
...
class RPC:
    ...
    def __init__(self, HuangTrader) -> None:
        """
        Initializes all enabled rpc modules
        :param HuangTrader: Instance of a HuangTrader bot
        :return: None
        """
        self._HuangTrader = HuangTrader
        self._config: Config = HuangTrader.config
    ...
    def _rpc_show_config(
        ...
        val = {
            ...
            "bot_name": config.get("bot_name", "HuangTrader"),
    ...
    def _rpc_trade_status(self, trade_ids: Optional[List[int]] = None) -> List[Dict[str, Any]]:
        ...
        if not trades:
            ...
            for trade in trades:
                ...
                if trade.is_open:
                    try:
                        current_rate = self._HuangTrader.exchange.get_rate(
                ...
                if not isnan(current_profit_abs) and self._fiat_converter:
                    current_profit_fiat = self._fiat_converter.convert_amount(
                        current_profit_abs,
                        self._HuangTrader.config["stake_currency"],
                        self._HuangTrader.config["fiat_display_currency"],
                    )
    ...
    def _rpc_timeunit_profit(
        ...
        daily_stake = self._HuangTrader.wallets.get_total_stake_amount()
    ...
    def _rpc_trade_statistics(
        ...
        starting_balance = self._HuangTrader.wallets.get_starting_balance()
    ...
    def __balance_get_est_stake(
        ...
        if coin == stake_currency:
            ...
            pair = self._HuangTrader.exchange.get_valid_pair_combination(coin, stake_currency)
    ...
    def _rpc_balance(self, stake_currency: str, fiat_display_currency: str) -> Dict:
        ...
        total_bot = 0.0
        try:
            tickers: Tickers = self._HuangTrader.exchange.get_tickers(cached=True)
        except ExchangeError:
            raise RPCException("Error getting current tickers.")

        open_trades: List[Trade] = Trade.get_open_trades()
        open_assets: Dict[str, Trade] = {t.safe_base_currency: t for t in open_trades}
        self._HuangTrader.wallets.update(require_update=False)
        starting_capital = self._HuangTrader.wallets.get_starting_balance()
        starting_cap_fiat = (
            self._fiat_converter.convert_amount(
        ...
        coin: str
        balance: Wallet
        for coin, balance in self._HuangTrader.wallets.get_all_balances().items():
            ...
            if coin == stake_currency:
                trade_amount = self._HuangTrader.wallets.get_available_stake_amount()
        ...
        for symbol, position in self._HuangTrader.wallets.get_all_positions().items():
        ...
        return {
            ...
            "note": "Simulated balances" if self._HuangTrader.config["dry_run"] else "",
        }

    def _rpc_start(self) -> Dict[str, str]:
        """Handler for start"""
        if self._HuangTrader.state == State.RUNNING:
            return {"status": "already running"}

        self._HuangTrader.state = State.RUNNING
        return {"status": "starting trader ..."}

    def _rpc_stop(self) -> Dict[str, str]:
        """Handler for stop"""
        if self._HuangTrader.state == State.RUNNING:
            self._HuangTrader.state = State.STOPPED
    ...
    def _rpc_reload_config(self) -> Dict[str, str]:
        ...
        self._HuangTrader.state = State.RELOAD_CONFIG
    ...
    def _rpc_stopentry(self) -> Dict[str, str]:
        ...
        if self._HuangTrader.state == State.RUNNING:
            ...
            self._HuangTrader.config["max_open_trades"] = 0
            self._HuangTrader.strategy.max_open_trades = 0
    ...
    def _rpc_reload_trade_from_exchange(self, trade_id: int) -> Dict[str, str]:
        ...
        self._HuangTrader.handle_onexchange_order(trade)
    ...
    def __exec_force_exit(
        ...
        for oo in trade.open_orders:
            ...
            order = self._HuangTrader.exchange.fetch_order(oo.order_id, trade.pair)

            if order["side"] == trade.entry_side:
                fully_canceled = self._HuangTrader.handle_cancel_enter(
            ...
            if order["side"] == trade.exit_side:
                ...
                self._HuangTrader.handle_cancel_exit(trade, order, oo, CANCEL_REASON["FORCE_EXIT"])
        ...
        if all(tocr["cancel_state"] is False for tocr in trade_entry_cancelation_registry):
            ...
            order_type = ordertype or self._HuangTrader.strategy.order_types.get(
                "force_exit", self._HuangTrader.strategy.order_types["exit"]
            )
            sub_amount: Optional[float] = None
            if amount and amount < trade.amount:
                # Partial exit ...
                min_exit_stake = self._HuangTrader.exchange.get_min_pair_stake_amount(
            ...
            self._HuangTrader.execute_trade_exit(
    ...
    def _force_entry_validations(self, pair: str, order_side: SignalDirection):
        if not self._HuangTrader.config.get("force_entry_enable", False):
            raise RPCException("Force_entry not enabled.")

        if self._HuangTrader.state != State.RUNNING:
            raise RPCException("trader is not running")

        if order_side == SignalDirection.SHORT and self._HuangTrader.trading_mode == TradingMode.SPOT:
            raise RPCException("Can't go short on Spot markets.")

        if pair not in self._HuangTrader.exchange.get_markets(tradable_only=True):
            raise RPCException("Symbol does not exist or market is not active.")
        # Check if pair quote currency equals to the stake currency.
        stake_currency = self._HuangTrader.config.get("stake_currency")
        if not self._HuangTrader.exchange.get_pair_quote_currency(pair) == stake_currency:
            raise RPCException(
    ...
    def _rpc_force_entry(
        ...
        *,
        ...
        if trade:
            ...
            if not self._HuangTrader.strategy.position_adjustment_enable:
        ...
        if not stake_amount:
            # gen stake amount
            stake_amount = self._HuangTrader.wallets.get_trade_stake_amount(
                pair, self._config["max_open_trades"]
            )

        # execute buy
        if not order_type:
            order_type = self._HuangTrader.strategy.order_types.get(
                "force_entry", self._HuangTrader.strategy.order_types["entry"]
            )
        with self._HuangTrader._exit_lock:
            if self._HuangTrader.execute_entry(
                pair,
                stake_amount,
                price,
    ...
    def _rpc_cancel_open_order(self, trade_id: int):
        ...
        with self._HuangTrader._exit_lock:
            ...
            for open_order in trade.open_orders:
                try:
                    order = self._HuangTrader.exchange.fetch_order(open_order.order_id, trade.pair)
                except ExchangeError as e:
                    logger.info(f"Cannot query order for {trade} due to {e}.", exc_info=True)
                    raise RPCException("Order not found.")
                self._HuangTrader.handle_cancel_order(
    ...
    def _rpc_delete(self, trade_id: int) -> Dict[str, Union[str, int]]:
        ...
        with self._HuangTrader._exit_lock:
            ...
            for open_order in trade.open_orders:
                try:
                    self._HuangTrader.exchange.cancel_order(open_order.order_id, trade.pair)
                    c_count += 1
                except ExchangeError:
                    pass
            ...
            if (
                self._HuangTrader.strategy.order_types.get("stoploss_on_exchange")
                and trade.has_open_sl_orders
            ):
                for oslo in trade.open_sl_orders:
                    try:
                        self._HuangTrader.exchange.cancel_stoploss_order(oslo.order_id, trade.pair)
                        c_count += 1
                    except ExchangeError:
                        pass
    ...
    def _rpc_count(self) -> Dict[str, float]:
        ...
        return {
            "current": len(trades),
            "max": (
                int(self._HuangTrader.config["max_open_trades"])
                if self._HuangTrader.config["max_open_trades"] != float("inf")
                else -1
            ),
    ...
    def _rpc_whitelist(self) -> Dict:
        ...
        res = {
            "method": self._HuangTrader.pairlists.name_list,
            "length": len(self._HuangTrader.active_pair_whitelist),
            "whitelist": self._HuangTrader.active_pair_whitelist,
        }
        return res
    ...
    def _rpc_blacklist_delete(self, delete: List[str]) -> Dict:
        ...
        errors = {}
        for pair in delete:
            if pair in self._HuangTrader.pairlists.blacklist:
                self._HuangTrader.pairlists.blacklist.remove(pair)
            else:
    ...
    def _rpc_blacklist(self, add: Optional[List[str]] = None) -> Dict:
        ...
        if add:
            for pair in add:
                if pair not in self._HuangTrader.pairlists.blacklist:
                    try:
                        expand_pairlist([pair], self._HuangTrader.exchange.get_markets().keys())
                        self._HuangTrader.pairlists.blacklist.append(pair)
        ...
        res = {
            "method": self._HuangTrader.pairlists.name_list,
            "length": len(self._HuangTrader.pairlists.blacklist),
            "blacklist": self._HuangTrader.pairlists.blacklist,
            "blacklist_expanded": self._HuangTrader.pairlists.expanded_blacklist,
            "errors": errors,
        }
        return res
    ...
    def _rpc_get_logs(limit: Optional[int]) -> Dict[str, Any]:
        ...
        #       "HuangTrader.worker", "INFO", "Starting worker develop"]

        return {"log_count": len(records), "logs": records}

    def _rpc_edge(self) -> List[Dict[str, Any]]:
        """Returns information related to Edge"""
        if not self._HuangTrader.edge:
            raise RPCException("Edge is not enabled.")
        return self._HuangTrader.edge.accepted_pairs()
    ...
    def _rpc_analysed_dataframe(
        ...
        return RPC._convert_dataframe_to_dict(
            self._HuangTrader.config["strategy"], pair, timeframe, _data, last_analyzed, selected_cols
        )
    ...
    def __rpc_analysed_dataframe_raw(
        ...
        """
        _data, last_analyzed = self._HuangTrader.dataprovider.get_analyzed_dataframe(pair, timeframe)
        _data = _data.copy()
    ...
    def _ws_all_analysed_dataframes(
        ...
        """
        timeframe = self._HuangTrader.config["timeframe"]
        candle_type = self._HuangTrader.config.get("candle_type_def", CandleType.SPOT)
    ...
    def _ws_request_analyzed_df(self, limit: Optional[int] = None, pair: Optional[str] = None):
        ...
        pairlist = [pair] if pair else self._HuangTrader.active_pair_whitelist

        return self._ws_all_analysed_dataframes(pairlist, limit)

    def _ws_request_whitelist(self):
        """Whitelist data for WebSocket"""
        return self._HuangTrader.active_pair_whitelist
    ...
    def _rpc_analysed_history_full(
        ...
        from HuangTrader.data.converter import trim_dataframe
    ...
    def _rpc_plot_config(self) -> Dict[str, Any]:
        if (
            self._HuangTrader.strategy.plot_config
            and "subplots" not in self._HuangTrader.strategy.plot_config
        ):
            self._HuangTrader.strategy.plot_config["subplots"] = {}
        return self._HuangTrader.strategy.plot_config
    ...
    def health(self) -> Dict[str, Optional[Union[str, int]]]:
        last_p = self._HuangTrader.last_process
    ...
    def _update_market_direction(self, direction: MarketDirection) -> None:
        self._HuangTrader.strategy.market_direction = direction

    def _get_market_direction(self) -> MarketDirection:
        return self._HuangTrader.strategy.market_direction
