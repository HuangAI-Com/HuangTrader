...
from HuangTrader.constants import DUST_PER_COIN, Config
from HuangTrader.enums import MarketDirection, RPCMessageType, SignalDirection, TradingMode
from HuangTrader.exceptions import OperationalException
from HuangTrader.misc import chunks, plural
from HuangTrader.persistence import Trade
from HuangTrader.rpc import RPC, RPCException, RPCHandler
from HuangTrader.rpc.rpc_types import RPCEntryMsg, RPCExitMsg, RPCOrderMsg, RPCSendMsg
from HuangTrader.util import dt_from_ts, dt_humanize_delta, fmt_coin, format_date, round_value
...
class Telegram(RPCHandler):
    ...
    def _add_analyzed_candle(self, pair: str) -> str:
        ...
        if candle_val != "off":
            if candle_val == "ohlc":
                analyzed_df, _ = self._rpc._HuangTrader.dataprovider.get_analyzed_dataframe(
    ...
    async def _help(self, update: Update, context: CallbackContext) -> None:
        ...
        if self._rpc._HuangTrader.trading_mode != TradingMode.SPOT:
            force_enter_text += (
    ...
    async def _version(self, update: Update, context: CallbackContext) -> None:
        ...
        :return: None
        """
        strategy_version = self._rpc._HuangTrader.strategy.version()
    ...
    async def _show_config(self, update: Update, context: CallbackContext) -> None:
        ...
        :return: None
        """
        val = RPC._rpc_show_config(self._config, self._rpc._HuangTrader.state)
