...
from HuangTrader.constants import PROCESS_THROTTLE_SECS, RETRY_TIMEOUT, Config
from HuangTrader.enums import RPCMessageType, State
from HuangTrader.exceptions import OperationalException, TemporaryError
from HuangTrader.exchange import timeframe_to_next_date
...
class Worker:
    ...
    def _init(self, reconfig: bool) -> None:
        ...
        self.HuangTrader = HuangTraderBot(self._config)
    ...
    def _worker(self, old_state: Optional[State]) -> State:
        ...
        """
        state = self.HuangTrader.state

        # Log state transition
        if state != old_state:
            if old_state != State.RELOAD_CONFIG:
                self.HuangTrader.notify_status(f"{state.name.lower()}")

            logger.info(
                f"Changing state{f' from {old_state.name}' if old_state else ''} to: {state.name}"
            )
            if state == State.RUNNING:
                self.HuangTrader.startup()

            if state == State.STOPPED:
                self.HuangTrader.check_for_open_trades()
        ...
        if self._heartbeat_interval:
            ...
            if (now - self._heartbeat_msg) > self._heartbeat_interval:
                version = __version__
                strategy_version = self.HuangTrader.strategy.version()
    ...
    def _process_stopped(self) -> None:
        self.HuangTrader.process_stopped()

    def _process_running(self) -> None:
        try:
            self.HuangTrader.process()
            ...
            self.HuangTrader.notify_status(
                f"*OperationalException:*\n```\n{tb}```\n {hint}", msg_type=RPCMessageType.EXCEPTION
            )

            logger.exception("OperationalException. Stopping trader ...")
            self.HuangTrader.state = State.STOPPED

    def _reconfigure(self) -> None:
        """
        Cleans up current HuangTraderbot instance, reloads the configuration and
        replaces it with the new instance
        """
        # Tell systemd that we initiated reconfiguration
        self._notify("RELOADING=1")

        # Clean up current HuangTrader modules
        self.HuangTrader.cleanup()

        # Load and validate config and create new instance of the bot
        self._init(True)

        self.HuangTrader.notify_status("config reloaded")
    ...
    def exit(self) -> None:
        ...
        if self.HuangTrader:
            self.HuangTrader.notify_status("process died")
            self.HuangTrader.cleanup()
