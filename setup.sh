...
function updateenv() {
    ...
    if [ $? -ne 0 ]; then
        echo "Failed installing HuangTrader"
        exit 1
    fi
...
function config() {
    echo_block "Please use 'HuangTrader new-config -c user_data/config.json' to generate a new configuration file."
}
...
function install() {
    ...
    echo "You can now use the bot by executing 'source .venv/bin/activate; HuangTrader <subcommand>'."
    echo "You can see the list of available bot sub-commands by executing 'source .venv/bin/activate; HuangTrader --help'."
    echo "You verify that HuangTrader is installed successfully by running 'source .venv/bin/activate; HuangTrader --version'."
}
...
function help() {
    echo "usage:"
    echo "	-i,--install    Install HuangTrader from scratch"
