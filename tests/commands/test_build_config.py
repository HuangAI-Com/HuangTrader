...
from HuangTrader.commands.build_config_commands import (
    ask_user_config,
    ask_user_overwrite,
    start_new_config,
    validate_is_float,
...
def test_start_new_config(mocker, caplog, exchange):
    ...
    mocker.patch("HuangTrader.commands.build_config_commands.ask_user_overwrite", return_value=True)
    ...
    sample_selections = {
        ...
        "api_server_username": "HuangTraderr",
        "api_server_password": "MoneyMachine",
    }
    mocker.patch(
        "HuangTrader.commands.build_config_commands.ask_user_config", return_value=sample_selections
    )
...
def test_start_new_config_exists(mocker, caplog):
    mocker.patch.object(Path, "exists", MagicMock(return_value=True))
    mocker.patch("HuangTrader.commands.build_config_commands.ask_user_overwrite", return_value=False)
    args = ["new-config", "--config", "coolconfig.json"]
...
def test_ask_user_overwrite(mocker):
    ...
    prompt_mock = mocker.patch(
        "HuangTrader.commands.build_config_commands.prompt", return_value={"overwrite": False}
    )
    ...
    prompt_mock = mocker.patch(
        "HuangTrader.commands.build_config_commands.prompt", return_value={"overwrite": True}
    )
...
def test_ask_user_config(mocker):
    ...
    prompt_mock = mocker.patch("HuangTrader.commands.build_config_commands.prompt", return_value={})
