...
from HuangTrader.commands import (
...
from HuangTrader.commands.list_commands import start_list_freqAI_models
...
from HuangTrader.util import dt_floor_day, dt_now, dt_utc
...
def test_start_trading_fail(mocker, caplog):
    mocker.patch("HuangTrader.worker.Worker.run", MagicMock(side_effect=OperationalException))

    mocker.patch("HuangTrader.worker.Worker.__init__", MagicMock(return_value=None))

    exitmock = mocker.patch("HuangTrader.worker.Worker.exit", MagicMock())
    ...
    exitmock.reset_mock()
    caplog.clear()
    mocker.patch("HuangTrader.worker.Worker.__init__", MagicMock(side_effect=OperationalException))
    with pytest.raises(OperationalException):
        start_trading(get_args(args))
...
def test_start_webserver(mocker, caplog):
    api_server_mock = mocker.patch(
        "HuangTrader.rpc.api_server.ApiServer",
    )
...
def test_list_exchanges(capsys):
    ...
    assert re.match(r"Exchanges available for HuangTrader.*", captured.out)
...
def test_create_datadir(caplog, mocker):
    cud = mocker.patch("HuangTrader.commands.deploy_commands.create_userdata_dir", MagicMock())
    csf = mocker.patch("HuangTrader.commands.deploy_commands.copy_sample_files", MagicMock())
    args = ["create-userdir", "--userdir", "/temp/HuangTrader/test"]
    start_create_userdir(get_args(args))
...
def test_start_new_strategy(mocker, caplog):
    ...
    mocker.patch("HuangTrader.commands.deploy_commands.setup_utils_configuration")
...
def test_start_install_ui(mocker):
    clean_mock = mocker.patch("HuangTrader.commands.deploy_commands.clean_ui_subdir")
    get_url_mock = mocker.patch(
        "HuangTrader.commands.deploy_commands.get_ui_download_url",
        return_value=("https://example.com/whatever", "0.0.1"),
    )
    download_mock = mocker.patch("HuangTrader.commands.deploy_commands.download_and_install_ui")
    mocker.patch("HuangTrader.commands.deploy_commands.read_ui_version", return_value=None)
    args = [
        "install-ui",
    ]
...
def test_clean_ui_subdir(mocker, tmp_path, caplog):
    mocker.patch("HuangTrader.commands.deploy_commands.Path.is_dir", side_effect=[True, True])
    mocker.patch("HuangTrader.commands.deploy_commands.Path.is_file", side_effect=[False, True])
    rd_mock = mocker.patch("HuangTrader.commands.deploy_commands.Path.rmdir")
    ul_mock = mocker.patch("HuangTrader.commands.deploy_commands.Path.unlink")

    mocker.patch(
        "HuangTrader.commands.deploy_commands.Path.glob",
...
def test_download_and_install_ui(mocker, tmp_path):
    ...
    mocker.patch("HuangTrader.commands.deploy_commands.requests.get", return_value=requests_mock)

    mocker.patch("HuangTrader.commands.deploy_commands.Path.is_dir", side_effect=[True, False])
    wb_mock = mocker.patch("HuangTrader.commands.deploy_commands.Path.write_bytes")
...
def test_get_ui_download_url(mocker):
    ...
    get_mock = mocker.patch(
        "HuangTrader.commands.deploy_commands.requests.get", return_value=response
    )
...
def test_download_data_keyboardInterrupt(mocker, markets):
    dl_mock = mocker.patch(
        "HuangTrader.commands.data_commands.download_data_main",
...
def test_download_data_timerange(mocker, markets):
    dl_mock = mocker.patch(
        "HuangTrader.data.history.history_utils.refresh_backtest_ohlcv_data",
...
def test_download_data_trades(mocker):
    dl_mock = mocker.patch(
        "HuangTrader.data.history.history_utils.refresh_backtest_trades_data",
        MagicMock(return_value=[]),
    )
    convert_mock = mocker.patch(
        "HuangTrader.data.history.history_utils.convert_trades_to_ohlcv", MagicMock(return_value=[])
    )
    patch_exchange(mocker)
    mocker.patch(f"{EXMS}.get_markets", return_value={})
    args = [
...
def test_start_convert_trades(mocker):
    convert_mock = mocker.patch(
        "HuangTrader.commands.data_commands.convert_trades_to_ohlcv", MagicMock(return_value=[])
    )
    patch_exchange(mocker)
...
def test_hyperopt_list(mocker, capsys, caplog, tmp_path):
    ...
    mocker.patch(
        "HuangTrader.optimize.hyperopt_tools.HyperoptTools._test_hyperopt_results_exist",
        return_value=True,
    )
    ...
    mocker.patch(
        "HuangTrader.optimize.hyperopt_tools.HyperoptTools._read_results", side_effect=fake_iterator
    )
...
def test_hyperopt_show(mocker, capsys):
    ...
    mocker.patch("HuangTrader.commands.hyperopt_commands.show_backtest_result")
...
def test_convert_data(mocker, testdatadir):
    ohlcv_mock = mocker.patch("HuangTrader.commands.data_commands.convert_ohlcv_format")
    trades_mock = mocker.patch("HuangTrader.commands.data_commands.convert_trades_format")
    args = [
        "convert-data",
        "--format-from",
        "json",
...
def test_show_trades(mocker, fee, capsys, caplog):
    mocker.patch("HuangTrader.persistence.init_db")
...
def test_backtesting_show(mocker, testdatadir, capsys):
    sbr = mocker.patch("HuangTrader.optimize.optimize_reports.show_backtest_results")
    args = [
        "backtesting-show",
        "--export-filename",
...
def test_start_strategy_updater(mocker, tmp_path):
    sc_mock = mocker.patch("HuangTrader.commands.strategy_utils_commands.start_conversion")
