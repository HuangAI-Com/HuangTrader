...
from HuangTrader.data.converter import ohlcv_to_dataframe, trades_list_to_df
from HuangTrader.edge import PairInfo
from HuangTrader.enums import CandleType, MarginMode, RunMode, SignalDirection, TradingMode
from HuangTrader.exchange import Exchange, timeframe_to_minutes, timeframe_to_seconds
from HuangTrader.HuangTraderbot import HuangTraderBot
from HuangTrader.persistence import LocalTrade, Order, Trade, init_db
from HuangTrader.resolvers import ExchangeResolver
from HuangTrader.util import dt_now, dt_ts
...
EXMS = "HuangTrader.exchange.exchange.Exchange"
...
def patched_configuration_load_config_file(mocker, config) -> None:
    mocker.patch(
        "HuangTrader.configuration.load_config.load_config_file", lambda *args, **kwargs: config
    )
...
def patch_exchange(
    ...
    # Temporary patch ...
    mocker.patch("HuangTrader.exchange.bybit.Bybit.cache_leverage_tiers")
    ...
    if mock_supported_modes:
        mocker.patch(
            f"HuangTrader.exchange.{id}.{id.capitalize()}._supported_trading_mode_margin_pairs",
            PropertyMock(
                return_value=[
...
def patch_wallet(mocker, free=999.9) -> None:
    mocker.patch("HuangTrader.wallets.Wallets.get_free", MagicMock(return_value=free))


def patch_whitelist(mocker, conf) -> None:
    mocker.patch(
        "HuangTrader.HuangTraderbot.HuangTraderBot._refresh_active_whitelist",
...
def patch_edge(mocker) -> None:
    ...
    mocker.patch("HuangTrader.edge.Edge.calculate", MagicMock(return_value=True))


# Functions for recurrent object patching


def patch_HuangTraderbot(mocker, config) -> None:
    """
    ...
    mocker.patch("HuangTrader.HuangTraderbot.RPCManager._init", MagicMock())
    mocker.patch("HuangTrader.HuangTraderbot.RPCManager.send_msg", MagicMock())
    patch_whitelist(mocker, config)
    mocker.patch("HuangTrader.HuangTraderbot.ExternalMessageConsumer")
    mocker.patch("HuangTrader.configuration.config_validation._validate_consumers")


def get_patched_HuangTraderbot(mocker, config) -> HuangTraderBot:
    """
    This function patches _init_modules() to not call dependencies
    :param mocker: a Mocker object to apply patches
    :param config: Config to pass to the bot
    :return: HuangTraderBot
    """
    patch_HuangTraderbot(mocker, config)
    return HuangTraderBot(config)
...
def patch_get_signal(
    ...
    HuangTrader.strategy.get_entry_signal = patched_get_entry_signal
    ...
    # returns (enter, exit)
    HuangTrader.strategy.get_exit_signal = patched_get_exit_signal

    HuangTrader.exchange.refresh_latest_ohlcv = lambda p: None
...
def patch_gc(mocker) -> None:
    mocker.patch("HuangTrader.main.gc_set_threshold")


@pytest.fixture(autouse=True)
def user_dir(mocker, tmp_path) -> Path:
    user_dir = tmp_path / "user_data"
    mocker.patch("HuangTrader.configuration.configuration.create_userdata_dir", return_value=user_dir)
    return user_dir
...
def patch_coingecko(mocker) -> None:
    ...
    mocker.patch.multiple(
        "HuangTrader.rpc.fiat_convert.FtCoinGeckoApi",
