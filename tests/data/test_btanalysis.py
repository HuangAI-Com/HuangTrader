...
from HuangTrader.constants import LAST_BT_RESULT_FN
...
from HuangTrader.data.history import load_data, load_pair_history
from HuangTrader.data.metrics import (
    calculate_cagr,
    calculate_calmar,
    calculate_csum,
...
def test_get_latest_backtest_filename(testdatadir, mocker):
    ...
    mocker.patch("HuangTrader.data.btanalysis.json_load", return_value={})
...
def test_load_backtest_metadata(mocker, testdatadir):
    ...
    mocker.patch("HuangTrader.data.btanalysis.get_backtest_metadata_filename")
    mocker.patch("HuangTrader.data.btanalysis.json_load", side_effect=Exception())
    with pytest.raises(
...
def test_load_backtest_data_old_format(testdatadir, mocker):
    ...
    mocker.patch("HuangTrader.data.btanalysis.load_backtest_stats", return_value=[])
...
def test_load_trades_from_db(default_conf, fee, is_short, mocker):
    ...
    init_mock = mocker.patch("HuangTrader.data.btanalysis.init_db", MagicMock())
...
def test_load_trades(default_conf, mocker):
    db_mock = mocker.patch("HuangTrader.data.btanalysis.load_trades_from_db", MagicMock())
    bt_mock = mocker.patch("HuangTrader.data.btanalysis.load_backtest_data", MagicMock())
...
def test_calculate_max_drawdown_abs(profits, relative, highd, lowdays, result, result_rel):
    """
    Test case from issue https://github.com/HuangTrader/HuangTrader/issues/6655
