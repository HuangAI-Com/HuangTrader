...
from HuangTrader.exceptions import ExchangeError, OperationalException
...
def test_historic_ohlcv(mocker, default_conf, ohlcv_history):
    ...
    mocker.patch("HuangTrader.data.dataprovider.load_pair_history", historymock)
...
def test_historic_ohlcv_dataformat(mocker, default_conf, ohlcv_history):
    ...
    mocker.patch(
        "HuangTrader.data.history.datahandlers.hdf5datahandler.HDF5DataHandler._ohlcv_load",
        hdf5loadmock,
    )
    mocker.patch(
...
def test_emit_df(mocker, default_conf, ohlcv_history):
    mocker.patch("HuangTrader.rpc.rpc_manager.RPCManager.__init__", MagicMock())
    rpc_mock = mocker.patch("HuangTrader.rpc.rpc_manager.RPCManager", MagicMock())
    send_mock = mocker.patch("HuangTrader.rpc.rpc_manager.RPCManager.send_msg", MagicMock())
...
def test_dp_get_required_startup(default_conf_usdt):
    ...
    # scenario from issue https://github.com/HuangTrader/HuangTrader/issues/9432
    dp._config["freqai"] = {
        "enabled": True,
