...
from HuangTrader.constants import DATETIME_PRINT_FORMAT
from HuangTrader.data.converter import ohlcv_to_dataframe
from HuangTrader.data.history import get_datahandler
from HuangTrader.data.history.datahandlers.jsondatahandler import JsonDataHandler, JsonGzDataHandler
from HuangTrader.data.history.history_utils import (
...
from HuangTrader.enums import CandleType, TradingMode
from HuangTrader.exchange import timeframe_to_minutes
from HuangTrader.misc import file_dump_json
from HuangTrader.resolvers import StrategyResolver
from HuangTrader.util import dt_ts, dt_utc
...
def test_load_data_7min_timeframe(caplog, testdatadir) -> None:
    ...
    assert log_has(
        ...
        "Use `HuangTrader download-data` to download the data",
        caplog,
    )
...
def test_load_data_startup_candles(mocker, testdatadir) -> None:
    ltfmock = mocker.patch(
        "HuangTrader.data.history.datahandlers.featherdatahandler.FeatherDataHandler._ohlcv_load",
...
@pytest.mark.parametrize(
    ...
    [
        ("ETH/BTC", "5m", "HuangTrader/hello/world/ETH_BTC-5m.json", ""),
        ("ETH/USDT", "1M", "HuangTrader/hello/world/ETH_USDT-1Mo.json", ""),
        ("Fabric Token/ETH", "5m", "HuangTrader/hello/world/Fabric_Token_ETH-5m.json", ""),
        ("ETHH20", "5m", "HuangTrader/hello/world/ETHH20-5m.json", ""),
        (".XBTBON2H", "5m", "HuangTrader/hello/world/_XBTBON2H-5m.json", ""),
        ("ETHUSD.d", "5m", "HuangTrader/hello/world/ETHUSD_d-5m.json", ""),
        ("ACC_OLD/BTC", "5m", "HuangTrader/hello/world/ACC_OLD_BTC-5m.json", ""),
        ("ETH/BTC", "5m", "HuangTrader/hello/world/futures/ETH_BTC-5m-mark.json", "mark"),
        ("ACC_OLD/BTC", "5m", "HuangTrader/hello/world/futures/ACC_OLD_BTC-5m-index.json", "index"),
    ],
)
def test_json_pair_data_filename(pair, timeframe, expected_result, candle_type):
    fn = JsonDataHandler._pair_data_filename(
        Path("HuangTrader/hello/world"), pair, timeframe, CandleType.from_string(candle_type)
    )
    assert isinstance(fn, Path)
    assert fn == Path(expected_result)
    fn = JsonGzDataHandler._pair_data_filename(
        Path("HuangTrader/hello/world"),
        pair,
        timeframe,
...
@pytest.mark.parametrize(
    ...
    [
        ("ETH/BTC", "", "HuangTrader/hello/world/ETH_BTC-trades.json"),
        ("ETH/USDT:USDT", "futures", "HuangTrader/hello/world/futures/ETH_USDT_USDT-trades.json"),
        ("Fabric Token/ETH", "", "HuangTrader/hello/world/Fabric_Token_ETH-trades.json"),
        ("ETHH20", "", "HuangTrader/hello/world/ETHH20-trades.json"),
        (".XBTBON2H", "", "HuangTrader/hello/world/_XBTBON2H-trades.json"),
        ("ETHUSD.d", "", "HuangTrader/hello/world/ETHUSD_d-trades.json"),
        ("ACC_OLD_BTC", "", "HuangTrader/hello/world/ACC_OLD_BTC-trades.json"),
    ],
)
def test_json_pair_trades_filename(pair, trading_mode, expected_result):
    fn = JsonDataHandler._pair_trades_filename(Path("HuangTrader/hello/world"), pair, trading_mode)
    assert isinstance(fn, Path)
    assert fn == Path(expected_result)

    fn = JsonGzDataHandler._pair_trades_filename(Path("HuangTrader/hello/world"), pair, trading_mode)
    assert isinstance(fn, Path)
    assert fn == Path(expected_result + ".gz")
...
def test_download_pair_history2(mocker, default_conf, testdatadir) -> None:
    ...
    json_dump_mock = mocker.patch(
        "HuangTrader.data.history.datahandlers.featherdatahandler.FeatherDataHandler.ohlcv_store",
        return_value=None,
    )
...
def test_refresh_backtest_ohlcv_data(
    ...
    dl_mock = mocker.patch("HuangTrader.data.history.history_utils._download_pair_history")
...
def test_download_data_no_markets(mocker, default_conf, caplog, testdatadir):
    dl_mock = mocker.patch(
        "HuangTrader.data.history.history_utils._download_pair_history", MagicMock()
    )
...
def test_refresh_backtest_trades_data(mocker, default_conf, markets, caplog, testdatadir):
    dl_mock = mocker.patch(
        "HuangTrader.data.history.history_utils._download_trades_history", MagicMock()
    )
    mocker.patch(f"{EXMS}.markets", PropertyMock(return_value=markets))
    mocker.patch.object(Path, "exists", MagicMock(return_value=True))
    mocker.patch.object(Path, "unlink", MagicMock())
