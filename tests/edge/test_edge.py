...
from HuangTrader.edge import Edge, PairInfo
from HuangTrader.enums import ExitType
from HuangTrader.exceptions import OperationalException
from HuangTrader.util.datetime_helpers import dt_ts, dt_utc
from tests.conftest import EXMS, get_patched_HuangTraderbot, log_has
...
def test_edge_results(edge_conf, mocker, caplog, data) -> None:
    ...
    """
    HuangTrader = get_patched_HuangTraderbot(mocker, edge_conf)
    edge = Edge(edge_conf, HuangTrader.exchange, HuangTrader.strategy)
...
def test_edge_process_downloaded_data(mocker, edge_conf):
    ...
    mocker.patch("HuangTrader.edge.edge_positioning.refresh_data", MagicMock())
    mocker.patch("HuangTrader.edge.edge_positioning.load_data", mocked_load_data)
    edge = Edge(edge_conf, HuangTrader.exchange, HuangTrader.strategy)
...
def test_edge_process_no_data(mocker, edge_conf, caplog):
    ...
    mocker.patch(f"{EXMS}.get_fee", MagicMock(return_value=0.001))
    mocker.patch("HuangTrader.edge.edge_positioning.refresh_data", MagicMock())
    mocker.patch("HuangTrader.edge.edge_positioning.load_data", MagicMock(return_value={}))
    edge = Edge(edge_conf, HuangTrader.exchange, HuangTrader.strategy)
...
def test_edge_process_no_trades(mocker, edge_conf, caplog):
    ...
    mocker.patch(
        "HuangTrader.edge.edge_positioning.refresh_data",
    )
    mocker.patch("HuangTrader.edge.edge_positioning.load_data", mocked_load_data)
    # Return empty
    mocker.patch("HuangTrader.edge.Edge._find_trades_for_stoploss_range", return_value=[])
    edge = Edge(edge_conf, HuangTrader.exchange, HuangTrader.strategy)
...
def test_edge_process_no_pairs(mocker, edge_conf, caplog):
    ...
    mocker.patch("HuangTrader.HuangTraderbot.validate_config_consistency")

    HuangTrader = get_patched_HuangTraderbot(mocker, edge_conf)
    fee_mock = mocker.patch(f"{EXMS}.get_fee", return_value=0.001)
    mocker.patch("HuangTrader.edge.edge_positioning.refresh_data")
    mocker.patch("HuangTrader.edge.edge_positioning.load_data", mocked_load_data)
    # Return empty
    mocker.patch("HuangTrader.edge.Edge._find_trades_for_stoploss_range", return_value=[])
    edge = Edge(edge_conf, HuangTrader.exchange, HuangTrader.strategy)
...
def test_edge_init_error(mocker, edge_conf):
    ...
    with pytest.raises(OperationalException, match="Edge works only with unlimited stake amount"):
        get_patched_HuangTraderbot(mocker, edge_conf)
...
def test_process_expectancy(mocker, edge_conf, fee, risk_reward_ratio, expectancy):
    ...
    HuangTrader.exchange.get_fee = get_fee
    edge = Edge(edge_conf, HuangTrader.exchange, HuangTrader.strategy)
...
def test_process_expectancy_remove_pumps(mocker, edge_conf, fee):
    ...
    HuangTrader.exchange.get_fee = fee
    edge = Edge(edge_conf, HuangTrader.exchange, HuangTrader.strategy)
