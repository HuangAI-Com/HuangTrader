...
def test_check_exchange(default_conf, caplog) -> None:
    # Test an officially supported by HuangTrader team exchange
    default_conf["runmode"] = RunMode.DRY_RUN
    default_conf.get("exchange").update({"name": "BINANCE"})
    assert check_exchange(default_conf)
    assert log_has_re(
        r"Exchange .* is officially supported by the HuangTrader development team\.", caplog
    )
    caplog.clear()
    ...
    assert check_exchange(default_conf)
    assert log_has_re(
        r"Exchange \"binance\" is officially supported by the HuangTrader development team\.", caplog
    )
    caplog.clear()
    ...
    assert check_exchange(default_conf)
    assert log_has_re(
        r"Exchange \"binanceus\" is officially supported by the HuangTrader development team\.",
        caplog,
    )
    caplog.clear()

    # Test an officially supported by HuangTrader team exchange - with remapping
    default_conf.get("exchange").update({"name": "okx"})
    assert check_exchange(default_conf)
    assert log_has_re(
        r"Exchange \"okx\" is officially supported by the HuangTrader development team\.", caplog
    )
    caplog.clear()
    ...
    assert log_has_re(
        ...
        r"by the HuangTrader development team\. .*",
        caplog,
    )
    caplog.clear()
    ...
    with pytest.raises(OperationalException, match=r"Exchange .* will not work with HuangTrader\..*"):
