...
from HuangTrader.constants import CANCEL_REASON, UNLIMITED_STAKE_AMOUNT
from HuangTrader.enums import (
    CandleType,
    ExitCheckTuple,
    ExitType,
    RPCMessageType,
...
from HuangTrader.exceptions import (
    DependencyException,
    ExchangeError,
...
from HuangTrader.persistence import Order, PairLocks, Trade
from HuangTrader.plugins.protections.iprotection import ProtectionReturn
from HuangTrader.util.datetime_helpers import dt_now, dt_utc
...
def patch_RPCManager(mocker) -> MagicMock:
    ...
    """
    mocker.patch("HuangTrader.rpc.telegram.Telegram", MagicMock())
    rpc_mock = mocker.patch("HuangTrader.HuangTraderbot.RPCManager.send_msg", MagicMock())
    return rpc_mock
...
def test_HuangTraderbot_state(mocker, default_conf_usdt, markets) -> None:
    mocker.patch(f"{EXMS}.markets", PropertyMock(return_value=markets))
    HuangTrader = get_patched_HuangTraderbot(mocker, default_conf_usdt)
    assert HuangTrader.state is State.RUNNING

    default_conf_usdt.pop("initial_state")
    HuangTrader = HuangTraderBot(default_conf_usdt)
    assert HuangTrader.state is State.STOPPED


def test_process_stopped(mocker, default_conf_usdt) -> None:
    HuangTrader = get_patched_HuangTraderbot(mocker, default_conf_usdt)
    coo_mock = mocker.patch("HuangTrader.HuangTraderbot.HuangTraderBot.cancel_all_open_orders")
    HuangTrader.process_stopped()
...
def test_process_calls_sendmsg(mocker, default_conf_usdt) -> None:
    HuangTrader = get_patched_HuangTraderbot(mocker, default_conf_usdt)
    HuangTrader.process()
    assert HuangTrader.rpc.process_msg_queue.call_count == 1


def test_bot_cleanup(mocker, default_conf_usdt, caplog) -> None:
    mock_cleanup = mocker.patch("HuangTrader.HuangTraderbot.Trade.commit")
    coo_mock = mocker.patch("HuangTrader.HuangTraderbot.HuangTraderBot.cancel_all_open_orders")
    HuangTrader = get_patched_HuangTraderbot(mocker, default_conf_usdt)
    HuangTrader.cleanup()
    ...
    HuangTrader.config["cancel_open_orders_on_exit"] = True
    HuangTrader.cleanup()
    assert coo_mock.call_count == 1


def test_bot_cleanup_db_errors(mocker, default_conf_usdt, caplog) -> None:
    mocker.patch("HuangTrader.HuangTraderbot.Trade.commit", side_effect=OperationalException())
    mocker.patch(
        "HuangTrader.HuangTraderbot.HuangTraderBot.check_for_open_trades",
        side_effect=OperationalException(),
    )
    HuangTrader = get_patched_HuangTraderbot(mocker, default_conf_usdt)
    HuangTrader.emc = MagicMock()
    HuangTrader.emc.shutdown = MagicMock()
    HuangTrader.cleanup()
    assert HuangTrader.emc.shutdown.call_count == 1
...
def test_order_dict(default_conf_usdt, mocker, runmode, caplog) -> None:
    ...
    HuangTrader = HuangTraderBot(conf)
    if runmode == RunMode.LIVE:
        assert not log_has_re(r".*stoploss_on_exchange .* dry-run", caplog)
    assert HuangTrader.strategy.order_types["stoploss_on_exchange"]
    ...
    HuangTrader = HuangTraderBot(conf)
    assert not HuangTrader.strategy.order_types["stoploss_on_exchange"]
    assert not log_has_re(r".*stoploss_on_exchange .* dry-run", caplog)
...
def test_get_trade_stake_amount(default_conf_usdt, mocker) -> None:
    ...
    result = HuangTrader.wallets.get_trade_stake_amount("ETH/USDT", 1)
...
def test_load_strategy_no_keys(default_conf_usdt, mocker, runmode, caplog) -> None:
    ...
    conf["runmode"] = runmode
    erm = mocker.patch("HuangTrader.HuangTraderbot.ExchangeResolver.load_exchange")

    HuangTrader = HuangTraderBot(conf)
    strategy_config = HuangTrader.strategy.config
    ...
    for i in range(0, max_open):
        if expected[i] is not None:
            ...
            with pytest.raises(DependencyException):
                HuangTrader.wallets.get_trade_stake_amount("ETH/USDT", 1)
...
def test_edge_called_in_process(mocker, edge_conf) -> None:
    ...
    patch_exchange(mocker)
    HuangTrader = HuangTraderBot(edge_conf)
    patch_get_signal(HuangTrader)
    HuangTrader.process()
    assert HuangTrader.active_pair_whitelist == ["NEO/BTC", "LTC/BTC"]
...
def test_edge_overrides_stake_amount(mocker, edge_conf) -> None:
    ...
    assert (
        HuangTrader.wallets.get_trade_stake_amount("NEO/BTC", 1, HuangTrader.edge)
        == (999.9 * 0.5 * 0.01) / 0.20
    )
    assert (
        HuangTrader.wallets.get_trade_stake_amount("LTC/BTC", 1, HuangTrader.edge)
        == (999.9 * 0.5 * 0.01) / 0.21
    )
...
def test_edge_overrides_stoploss(
    ...
    HuangTrader = HuangTraderBot(edge_conf)
    HuangTrader.active_pair_whitelist = ["NEO/BTC"]
    patch_get_signal(HuangTrader)
    HuangTrader.strategy.min_roi_reached = MagicMock(return_value=False)
    HuangTrader.enter_positions()
    ...
    assert HuangTrader.handle_trade(trade) is not ignore_strat_sl
...
def test_create_trade(
    ...
    HuangTrader = HuangTraderBot(default_conf_usdt)
    patch_get_signal(HuangTrader, enter_short=is_short, enter_long=not is_short)
    HuangTrader.create_trade("ETH/USDT")
...
def test_create_trade_minimal_amount(
    ...
    HuangTrader = HuangTraderBot(default_conf_usdt)
    HuangTrader.config["stake_amount"] = stake_amount
    patch_get_signal(HuangTrader, enter_short=is_short, enter_long=not is_short)

    if create:
        assert HuangTrader.create_trade("ETH/USDT")
        if amount_enough:
        ...
        assert not HuangTrader.create_trade("ETH/USDT")
        if not max_open_trades:
            assert (
                HuangTrader.wallets.get_trade_stake_amount(
                    "ETH/USDT", default_conf_usdt["max_open_trades"], HuangTrader.edge
                )
                == 0
            )
...
def test_enter_positions_no_pairs_left(
    ...
    mocker.patch("HuangTrader.configuration.config_validation._validate_whitelist")
    default_conf_usdt["exchange"]["pair_whitelist"] = whitelist
    HuangTrader = HuangTraderBot(default_conf_usdt)
    patch_get_signal(HuangTrader)

    n = HuangTrader.enter_positions()
    assert n == positions
    if positions:
...
def test_handle_protections(mocker, default_conf_usdt, fee, is_short):
    ...
    HuangTrader = get_patched_HuangTraderbot(mocker, default_conf_usdt)
    HuangTrader.protections._protection_handlers[1].global_stop = MagicMock(
        return_value=ProtectionReturn(True, dt_now() + timedelta(hours=1), "asdf")
    )
    create_mock_trades(fee, is_short)
    HuangTrader.handle_protections("ETC/BTC", "*")
    send_msg_mock = HuangTrader.rpc.send_msg
...
def test_create_trade_no_signal(default_conf_usdt, fee, mocker) -> None:
    ...
    HuangTrader = HuangTraderBot(default_conf_usdt)
    patch_get_signal(HuangTrader, enter_long=False, exit_long=False)
...
def test_create_trades_preopen(
    ...
    HuangTrader.execute_entry("ETH/USDT", default_conf_usdt["stake_amount"])
    HuangTrader.execute_entry("NEO/BTC", default_conf_usdt["stake_amount"])
    ...
    assert HuangTrader.create_trade("ETH/USDT")
    assert HuangTrader.create_trade("NEO/BTC")
...
def test_process_exchange_failures(default_conf_usdt, ticker_usdt, mocker) -> None:
    ...
    patch_get_signal(worker.HuangTrader)
...
def test_process_operational_exception(default_conf_usdt, ticker_usdt, mocker) -> None:
    ...
    assert worker.HuangTrader.state == State.RUNNING

    worker._process_running()
    assert worker.HuangTrader.state == State.STOPPED
...
def test_process_trade_no_whitelist_pair(
    ...
    assert pair not in HuangTrader.active_pair_whitelist
    HuangTrader.process()
    assert pair in HuangTrader.active_pair_whitelist
    # Make sure each pair is only in the list once
    assert len(HuangTrader.active_pair_whitelist) == len(set(HuangTrader.active_pair_whitelist))
...
def test_process_informative_pairs_added(default_conf_usdt, ticker_usdt, mocker) -> None:
    ...
    mocker.patch.multiple(
        "HuangTrader.strategy.interface.IStrategy",
    ...
    HuangTrader = HuangTraderBot(default_conf_usdt)
    HuangTrader.strategy.informative_pairs = inf_pairs
    # patch_get_signal(HuangTrader)
...
def test_execute_entry(
    ...
    mocker.patch("HuangTrader.exchange.gate.Gate.validate_ordertypes")
    patch_RPCManager(mocker)
    patch_exchange(mocker, id=exchange_name)
    HuangTrader = HuangTraderBot(default_conf_usdt)
    HuangTrader.strategy.confirm_trade_entry = MagicMock(return_value=False)
    HuangTrader.strategy.leverage = MagicMock(return_value=leverage)
    stake_amount = 2
    bid = 0.11
    ...
    mocker.patch.multiple(
        "HuangTrader.exchange.okx.Okx",
        get_max_pair_stake_amount=MagicMock(return_value=500000),
    )
    pair = "ETH/USDT"

    assert not HuangTrader.execute_entry(pair, stake_amount, is_short=is_short)
    assert enter_rate_mock.call_count == 1
    assert enter_mm.call_count == 0
    assert HuangTrader.strategy.confirm_trade_entry.call_count == 1
    enter_rate_mock.reset_mock()

    open_order["id"] = "22"
    HuangTrader.strategy.confirm_trade_entry = MagicMock(return_value=True)
    assert HuangTrader.execute_entry(pair, stake_amount)
    ...
    open_order["id"] = "33"
    fix_price = 0.06
    assert HuangTrader.execute_entry(pair, stake_amount, fix_price, is_short=is_short)
    ...
    assert HuangTrader.execute_entry(pair, stake_amount, is_short=is_short)
    ...
    HuangTrader.strategy.custom_stake_amount = lambda **kwargs: 150.0
    assert HuangTrader.execute_entry(pair, stake_amount, is_short=is_short)
    ...
    # Exception case
    order["id"] = "557"
    HuangTrader.strategy.custom_stake_amount = lambda **kwargs: 20 / 0
    assert HuangTrader.execute_entry(pair, stake_amount, is_short=is_short)
    ...
    order["id"] = "66"
    mocker.patch(f"{EXMS}.create_order", MagicMock(return_value=order))
    assert not HuangTrader.execute_entry(pair, stake_amount)
    assert HuangTrader.strategy.leverage.call_count == 0 if trading_mode == "spot" else 2
    ...
    with pytest.raises(PricingError, match="Could not determine entry price."):
        HuangTrader.execute_entry(pair, stake_amount, is_short=is_short)

    # In case of custom entry price
    mocker.patch(f"{EXMS}.get_rate", return_value=0.50)
    order["status"] = "open"
    order["id"] = "5566"
    HuangTrader.strategy.custom_entry_price = lambda **kwargs: 0.508
    assert HuangTrader.execute_entry(pair, stake_amount, is_short=is_short)
    ...
    order["status"] = "open"
    order["id"] = "5567"
    HuangTrader.strategy.custom_entry_price = lambda **kwargs: None
    ...
    order["status"] = "open"
    order["id"] = "5568"
    HuangTrader.strategy.custom_entry_price = lambda **kwargs: "string price"
    assert HuangTrader.execute_entry(pair, stake_amount, is_short=is_short)
    ...
    HuangTrader.exchange.get_max_pair_stake_amount = MagicMock(return_value=500)

    assert HuangTrader.execute_entry(pair, 2000, is_short=is_short)
    ...
    HuangTrader.strategy.leverage.reset_mock()
    assert HuangTrader.execute_entry(pair, 200, leverage_=3)
    assert HuangTrader.strategy.leverage.call_count == 0
...
def test_execute_entry_confirm_error(mocker, default_conf_usdt, fee, limit_order, is_short) -> None:
    ...
    HuangTrader.strategy.confirm_trade_entry = MagicMock(side_effect=ValueError)
    assert HuangTrader.execute_entry(pair, stake_amount)

    limit_order[entry_side(is_short)]["id"] = "222"
    HuangTrader.strategy.confirm_trade_entry = MagicMock(side_effect=Exception)
    assert HuangTrader.execute_entry(pair, stake_amount)
...
def test_execute_entry_fully_canceled_on_create(
    ...
    mock_hce = mocker.spy(HuangTrader, "handle_cancel_enter")
...
def test_execute_entry_min_leverage(mocker, default_conf_usdt, fee, limit_order, is_short) -> None:
    ...
    stake_amount = 2
    pair = "SOL/BUSD:BUSD"
    HuangTrader.strategy.leverage = MagicMock(return_value=5.0)
...
def test_enter_positions(
    ...
    mock_ct = mocker.patch(
        "HuangTrader.HuangTraderbot.HuangTraderBot.create_trade",
...
def test_exit_positions(mocker, default_conf_usdt, limit_order, is_short, caplog) -> None:
    ...
    mocker.patch("HuangTrader.HuangTraderbot.HuangTraderBot.handle_trade", MagicMock(return_value=True))
    ...
    n = HuangTrader.exit_positions(trades)
    assert n == 0
    # Test amount not modified by fee-logic
    assert not log_has_re(r"Applying fee to amount for Trade .*", caplog)

    gra = mocker.patch("HuangTrader.HuangTraderbot.HuangTraderBot.get_real_amount", return_value=0.0)
...
def test_exit_positions_exception(mocker, default_conf_usdt, limit_order, caplog, is_short) -> None:
    ...
    mocker.patch(
        "HuangTrader.HuangTraderbot.HuangTraderBot.handle_trade", side_effect=DependencyException()
    )
    caplog.clear()
    n = HuangTrader.exit_positions(trades)
    assert n == 0
...
def test_update_trade_state(mocker, default_conf_usdt, limit_order, is_short, caplog) -> None:
    ...
    mocker.patch("HuangTrader.HuangTraderbot.HuangTraderBot.handle_trade", MagicMock(return_value=True))
    mocker.patch("HuangTrader.HuangTraderbot.HuangTraderBot._notify_enter")
    mocker.patch(f"{EXMS}.fetch_order", return_value=order)
    mocker.patch(f"{EXMS}.get_trades_for_order", return_value=[])
    mocker.patch("HuangTrader.HuangTraderbot.HuangTraderBot.get_real_amount", return_value=0.0)
    order_id = order["id"]
    ...
    HuangTrader.strategy.order_filled = MagicMock(return_value=None)
    assert not HuangTrader.update_trade_state(trade, None)
    assert log_has_re(r"Orderid for trade .* is empty.", caplog)
    caplog.clear()
    # Add datetime explicitly since sqlalchemy defaults apply only once written to database
    HuangTrader.update_trade_state(trade, order_id)
    # Test amount not modified by fee-logic
    assert not log_has_re(r"Applying fee to .*", caplog)
    caplog.clear()
    assert not trade.has_open_orders
    assert trade.amount == order["amount"]
    assert HuangTrader.strategy.order_filled.call_count == 1

    mocker.patch("HuangTrader.HuangTraderbot.HuangTraderBot.get_real_amount", return_value=0.01)
    assert trade.amount == 30.0
    # test amount modified by fee-logic
    HuangTrader.update_trade_state(trade, order_id)
    ...
    HuangTrader.strategy.order_filled = MagicMock(return_value=None)
    mocker.patch("HuangTrader.HuangTraderbot.HuangTraderBot.get_real_amount", side_effect=ValueError)
    mocker.patch(f"{EXMS}.fetch_order", return_value=limit_buy_order_usdt_new)
    res = HuangTrader.update_trade_state(trade, order_id)
    # Cancelled empty
    assert res is True
    assert HuangTrader.strategy.order_filled.call_count == 0
...
def test_update_trade_state_withorderdict(
    ...
    HuangTrader.update_trade_state(trade, order_id, order)
...
def test_update_trade_state_exception(
    ...
    mocker.patch(
        "HuangTrader.HuangTraderbot.HuangTraderBot.get_real_amount", side_effect=DependencyException()
    )
    HuangTrader.update_trade_state(trade, open_order_id)
...
def test_update_trade_state_orderexception(mocker, default_conf_usdt, caplog) -> None:
    ...
    # Test raise of OperationalException exception
    grm_mock = mocker.patch("HuangTrader.HuangTraderbot.HuangTraderBot.get_real_amount", MagicMock())
    HuangTrader.update_trade_state(trade, open_order_id)
...
def test_update_trade_state_sell(
    ...
    mocker.patch("HuangTrader.wallets.Wallets.update", wallet_mock)
    ...
    HuangTrader.update_trade_state(trade, trade.open_orders_ids[-1], l_order)
...
def test_handle_trade(
    ...
    assert HuangTrader.handle_trade(trade) is True
...
def test_handle_overlapping_signals(
    ...
    HuangTrader = HuangTraderBot(default_conf_usdt)
    if is_short:
        patch_get_signal(HuangTrader, enter_long=False, enter_short=True, exit_short=True)
    else:
        patch_get_signal(HuangTrader, enter_long=True, exit_long=True)
    HuangTrader.strategy.min_roi_reached = MagicMock(return_value=False)
    ...
    patch_get_signal(HuangTrader, enter_long=False)
    assert HuangTrader.handle_trade(trades[0]) is False
    trades = Trade.session.scalars(select(Trade)).all()
    for trade in trades:
        trade.is_short = is_short
    nb_trades = len(trades)
    ...
    if is_short:
        patch_get_signal(HuangTrader, enter_long=False, exit_short=True)
    else:
        patch_get_signal(HuangTrader, enter_long=False, exit_long=True)
    trades = Trade.session.scalars(select(Trade)).all()
    for trade in trades:
        trade.is_short = is_short
    assert HuangTrader.handle_trade(trades[0]) is True
...
def test_handle_trade_roi(
    ...
    HuangTrader = get_patched_HuangTraderbot(mocker, default_conf_usdt)
    patch_get_signal(HuangTrader, enter_short=is_short, enter_long=not is_short)
    HuangTrader.strategy.min_roi_reached = MagicMock(return_value=True)
    ...
    caplog.clear()
    patch_get_signal(HuangTrader)
    assert HuangTrader.handle_trade(trade)
...
def test_handle_trade_use_exit_signal(
    ...
    patch_get_signal(HuangTrader, enter_long=False, exit_long=False)
    assert not HuangTrader.handle_trade(trade)
...
def test_close_trade(
    ...
    with pytest.raises(DependencyException, match=r".*closed trade.*"):
        HuangTrader.handle_trade(trade)


def test_bot_loop_start_called_once(mocker, default_conf_usdt, caplog):
    ftbot = get_patched_HuangTraderbot(mocker, default_conf_usdt)
    mocker.patch("HuangTrader.HuangTraderbot.HuangTraderBot.create_trade")
...
def test_manage_open_orders_entry_usercustom(
    ...
    HuangTrader.manage_open_orders()
    assert cancel_order_mock.call_count == 0

    # Return false - trade remains open
    HuangTrader.strategy.check_entry_timeout = MagicMock(return_value=False)
    HuangTrader.manage_open_orders()
    assert cancel_order_mock.call_count == 0
    ...
    nb_trades = len(trades)
    assert nb_trades == 1
    assert HuangTrader.strategy.check_entry_timeout.call_count == 1
    HuangTrader.strategy.check_entry_timeout = MagicMock(side_effect=KeyError)
    ...
    assert nb_trades == 1
    assert HuangTrader.strategy.check_entry_timeout.call_count == 1
    HuangTrader.strategy.check_entry_timeout = MagicMock(return_value=True)
...
def test_manage_open_orders_entry(
    ...
    HuangTrader.strategy.check_entry_timeout = MagicMock(return_value=False)
    HuangTrader.strategy.adjust_entry_price = MagicMock(return_value=1234)
    ...
    assert HuangTrader.strategy.check_entry_timeout.call_count == 0
    # Entry adjustment is never called
    assert HuangTrader.strategy.adjust_entry_price.call_count == 0
...
def test_adjust_entry_cancel(
    ...
    HuangTrader.strategy.ft_check_timed_out = MagicMock(return_value=False)

    # check that order is cancelled
    HuangTrader.strategy.adjust_entry_price = MagicMock(return_value=None)
    HuangTrader.manage_open_orders()
    ...
    assert HuangTrader.strategy.adjust_entry_price.call_count == 1
...
def test_adjust_entry_replace_fail(
    ...
    mocker.patch("HuangTrader.HuangTraderbot.sleep")
    ...
    HuangTrader.strategy.adjust_entry_price = MagicMock(return_value=12234)
    HuangTrader.manage_open_orders()
    trades = Trade.session.scalars(select(Trade).where(Order.ft_trade_id == Trade.id)).all()
...
def test_adjust_entry_replace_fail_create_order(
    ...
    mocker.patch("HuangTrader.HuangTraderbot.sleep")
    mocker.patch(
        "HuangTrader.HuangTraderbot.HuangTraderBot.execute_entry", side_effect=DependencyException()
    )
...
def test_adjust_entry_maintain_replace(
    ...
    HuangTrader.strategy.adjust_entry_price = MagicMock(return_value=old_order["price"])
    HuangTrader.manage_open_orders()
    trades = Trade.session.scalars(
    ...
    HuangTrader.get_valid_enter_price_and_stake = MagicMock(return_value={100, 10, 1})
    HuangTrader.strategy.adjust_entry_price = MagicMock(return_value=1234)
...
def test_manage_open_orders_exit_usercustom(
    ...
    et_mock = mocker.patch("HuangTrader.HuangTraderbot.HuangTraderBot.execute_trade_exit")
    mocker.patch.multiple(
        EXMS,
        fetch_ticker=ticker_usdt,
    ...
    HuangTrader.strategy.check_exit_timeout = MagicMock(return_value=False)
    HuangTrader.strategy.check_entry_timeout = MagicMock(return_value=False)
    # Return false - No impact
    HuangTrader.manage_open_orders()
    assert cancel_order_mock.call_count == 0
    assert rpc_mock.call_count == 1
    assert HuangTrader.strategy.check_exit_timeout.call_count == 1
    assert HuangTrader.strategy.check_entry_timeout.call_count == 0

    HuangTrader.strategy.check_exit_timeout = MagicMock(side_effect=KeyError)
    HuangTrader.strategy.check_entry_timeout = MagicMock(side_effect=KeyError)
    ...
    # Return True - sells!
    HuangTrader.strategy.check_exit_timeout = MagicMock(return_value=True)
    HuangTrader.strategy.check_entry_timeout = MagicMock(return_value=True)
    HuangTrader.manage_open_orders()
    assert cancel_order_mock.call_count == 1
    assert rpc_mock.call_count == 2
    ...
    mocker.patch("HuangTrader.persistence.Trade.get_canceled_exit_order_count", return_value=1)
    mocker.patch(
        "HuangTrader.HuangTraderbot.HuangTraderBot.execute_trade_exit", side_effect=DependencyException
    )
    HuangTrader.manage_open_orders()
    ...
    with patch("HuangTrader.HuangTraderbot.HuangTraderBot.handle_cancel_exit", return_value=False):
        HuangTrader.manage_open_orders()
...
def test_manage_open_orders_exit(
    ...
    assert HuangTrader.strategy.check_exit_timeout.call_count == 0
    assert HuangTrader.strategy.check_entry_timeout.call_count == 0
...
def test_manage_open_orders_partial_fee(
    ...
    mocker.patch("HuangTrader.wallets.Wallets.get_free", MagicMock(return_value=0))
    patch_exchange(mocker)
    mocker.patch.multiple(
        EXMS,
        fetch_ticker=ticker_usdt,
...
def test_manage_open_orders_partial_except(
    ...
    mocker.patch(
        "HuangTrader.HuangTraderbot.HuangTraderBot.get_real_amount",
...
def test_manage_open_orders_exception(
    ...
    mocker.patch.multiple(
        "HuangTrader.HuangTraderbot.HuangTraderBot",
...
def test_handle_cancel_enter(mocker, caplog, default_conf_usdt, limit_order, is_short, fee) -> None:
    ...
    HuangTrader = HuangTraderBot(default_conf_usdt)
    HuangTrader._notify_enter_cancel = MagicMock()
    ...
    assert HuangTrader.handle_cancel_enter(trade, l_order, trade.open_orders[0], reason)
    assert cancel_order_mock.call_count == 1

    cancel_order_mock.reset_mock()
    caplog.clear()
    l_order["filled"] = 0.01
    assert not HuangTrader.handle_cancel_enter(trade, l_order, trade.open_orders[0], reason)
    assert cancel_order_mock.call_count == 0
    ...
    assert not HuangTrader.handle_cancel_enter(
...
def test_handle_cancel_enter_exchanges(
    ...
    notify_mock = mocker.patch("HuangTrader.HuangTraderbot.HuangTraderBot._notify_enter_cancel")
    HuangTrader = HuangTraderBot(default_conf_usdt)
    ...
    Trade.session.add(trade)
    Trade.commit()
    assert HuangTrader.handle_cancel_enter(
...
def test_handle_cancel_exit_limit(
    ...
    mocker.patch("HuangTrader.HuangTraderbot.HuangTraderBot.handle_order_fee")
    ...
    assert HuangTrader.handle_cancel_exit(trade, order, order_obj, reason)
    assert cancel_order_mock.call_count == 1
    ...
    assert not HuangTrader.handle_cancel_exit(trade, order, order_obj, reason)
...
def test_handle_cancel_exit_cancel_exception(mocker, default_conf_usdt) -> None:
    ...
    # assert not HuangTrader.handle_cancel_exit(trade, order, reason)
...
def test_execute_trade_exit_up(
    ...
    HuangTrader = HuangTraderBot(default_conf_usdt)
    patch_get_signal(HuangTrader, enter_short=is_short, enter_long=not is_short)
    HuangTrader.strategy.confirm_trade_exit = MagicMock(return_value=False)
    ...
    assert trade
    assert HuangTrader.strategy.confirm_trade_exit.call_count == 0
    ...
    # Prevented sell ...
    HuangTrader.execute_trade_exit(
        trade=trade,
    ...
    assert HuangTrader.strategy.confirm_trade_exit.call_count == 1
    assert id(HuangTrader.strategy.confirm_trade_exit.call_args_list[0][1]["trade"]) != id(trade)
    assert HuangTrader.strategy.confirm_trade_exit.call_args_list[0][1]["trade"].id == trade.id

    # Repatch with true
    HuangTrader.strategy.confirm_trade_exit = MagicMock(return_value=True)
    HuangTrader.execute_trade_exit(
        trade=trade,
        limit=(ticker_usdt_sell_down()["ask"] if is_short else ticker_usdt_sell_up()["bid"]),
        exit_check=ExitCheckTuple(exit_type=ExitType.ROI),
...
def test_execute_trade_exit_custom_exit_price(
    ...
    HuangTrader = HuangTraderBot(config)
    patch_get_signal(HuangTrader, enter_short=is_short, enter_long=not is_short)
    HuangTrader.strategy.confirm_trade_exit = MagicMock(return_value=False)
    ...
    HuangTrader.strategy.custom_exit_price = lambda **kwargs: 2.25
    HuangTrader.execute_trade_exit(
        trade=trade,
...
def test_execute_trade_exit_market_order(
    ...
    HuangTrader.config["order_types"]["exit"] = "market"
...
def test_execute_trade_exit_insufficient_funds_error(
    ...
    HuangTrader = get_patched_HuangTraderbot(mocker, default_conf_usdt)
    mock_insuf = mocker.patch("HuangTrader.HuangTraderbot.HuangTraderBot.handle_insufficient_funds")
    mocker.patch.multiple(
        EXMS,
        fetch_ticker=ticker_usdt,
        get_fee=fee,
    ...
    assert not HuangTrader.execute_trade_exit(
        trade=trade,
        limit=ticker_usdt_sell_up()["ask" if is_short else "bid"],
...
def test_exit_profit_only(
    ...
    HuangTrader = HuangTraderBot(default_conf_usdt)
    patch_get_signal(HuangTrader, enter_short=is_short, enter_long=not is_short)
    HuangTrader.strategy.custom_exit = MagicMock(return_value=None)
    if exit_type == ExitType.EXIT_SIGNAL.value:
        HuangTrader.strategy.min_roi_reached = MagicMock(return_value=False)
    else:
        HuangTrader.strategy.ft_stoploss_reached = MagicMock(
    ...
    HuangTrader.wallets.update()
    if profit_only:
        assert HuangTrader.handle_trade(trade) is False
        # Custom-exit is called
        assert HuangTrader.strategy.custom_exit.call_count == 1

    patch_get_signal(HuangTrader, enter_long=False, exit_short=is_short, exit_long=not is_short)
    assert HuangTrader.handle_trade(trade) is handle_first

    if handle_second:
        HuangTrader.strategy.exit_profit_offset = 0.0
        assert HuangTrader.handle_trade(trade) is True
...
def test_sell_not_enough_balance(
    ...
    trade.update_trade(oobj)
    patch_get_signal(HuangTrader, enter_long=False, exit_long=True)
    mocker.patch("HuangTrader.wallets.Wallets.get_free", MagicMock(return_value=trade.amount * 0.985))
...
def test__safe_exit_amount(default_conf_usdt, fee, caplog, mocker, amount_wallet, has_err):
    ...
    mocker.patch("HuangTrader.wallets.Wallets.get_free", MagicMock(return_value=amount_wallet))
    wallet_update = mocker.patch("HuangTrader.wallets.Wallets.update")
    trade = Trade(
        pair="LTC/ETH",
        amount=amount,
        exchange="binance",
    ...
    if has_err:
        with pytest.raises(DependencyException, match=r"Not enough amount to exit trade."):
            assert HuangTrader._safe_exit_amount(trade, trade.pair, trade.amount)
    else:
        wallet_update.reset_mock()
        assert trade.amount != amount_wallet
        assert HuangTrader._safe_exit_amount(trade, trade.pair, trade.amount) == amount_wallet
        assert log_has_re(r".*Falling back to wallet-amount.", caplog)
        assert trade.amount == amount_wallet
        assert wallet_update.call_count == 1
        caplog.clear()
        wallet_update.reset_mock()
        assert HuangTrader._safe_exit_amount(trade, trade.pair, amount_wallet) == amount_wallet
...
def test_locked_pairs(
    ...
    assert HuangTrader.strategy.is_pair_locked(trade.pair, side="*")
    # Both sides are locked
    assert HuangTrader.strategy.is_pair_locked(trade.pair, side="long")
    assert HuangTrader.strategy.is_pair_locked(trade.pair, side="short")
...
def test_ignore_roi_if_entry_signal(
    ...
    if is_short:
        patch_get_signal(HuangTrader, enter_long=False, exit_short=False, exit_tag="something")
    else:
        patch_get_signal(HuangTrader, enter_long=False, exit_long=False, exit_tag="something")
    assert HuangTrader.handle_trade(trade) is True
...
def test_disable_ignore_roi_if_entry_signal(
    ...
    patch_get_signal(HuangTrader, enter_long=not is_short, enter_short=is_short, exit_short=is_short)
    assert HuangTrader.handle_trade(trade) is True
...
def test_get_real_amount_quote(
    ...
    assert HuangTrader.get_real_amount(trade, buy_order_fee, order_obj) == (amount * 0.001)
    assert log_has(
...
def test_get_real_amount_quote_dust(
    ...
    mocker.patch(f"{EXMS}.get_trades_for_order", return_value=trades_for_order)
    walletmock = mocker.patch("HuangTrader.wallets.Wallets.update")
    mocker.patch("HuangTrader.wallets.Wallets.get_free", return_value=8.1122)
    amount = sum(x["amount"] for x in trades_for_order)
    trade = Trade(
        pair="LTC/ETH",
        amount=amount,
    ...
    assert HuangTrader.get_real_amount(trade, buy_order_fee, order_obj) is None
...
def test_get_real_amount(
    ...
    caplog.clear()
    order_obj = Order.parse_from_ccxt_object(buy_order_fee, "LTC/ETH", "buy")
    res = HuangTrader.get_real_amount(trade, buy_order, order_obj)
...
def test_get_real_amount_multi(
    ...
    order_obj = Order.parse_from_ccxt_object(buy_order_fee, "LTC/ETH", "buy")
    assert HuangTrader.get_real_amount(trade, buy_order_fee, order_obj) == expected_amount
    assert log_has(
        (
            "Applying fee on amount for Trade(id=None, pair=LTC/ETH, amount=8.00000000, "
...
def test_get_real_amount_invalid_order(
    ...
    assert HuangTrader.get_real_amount(trade, limit_buy_order_usdt, order_obj) is None
...
def test_get_real_amount_fees_order(
    ...
    assert HuangTrader.get_real_amount(trade, market_buy_order_usdt_doublefee, order_obj) is None
...
def test_get_real_amount_wrong_amount(
    ...
    with pytest.raises(DependencyException, match=r"Half bought\? Amounts don't match"):
        HuangTrader.get_real_amount(trade, limit_buy_order_usdt, order_obj)
...
def test_get_real_amount_wrong_amount_rounding(
    ...
    assert pytest.approx(HuangTrader.get_real_amount(trade, limit_buy_order_usdt, order_obj)) == (
        amount * 0.001
    )
...
def test_get_real_amount_open_trade_usdt(default_conf_usdt, fee, mocker):
    ...
    assert HuangTrader.get_real_amount(trade, order, order_obj) is None
...
def test_get_real_amount_in_point(default_conf_usdt, buy_order_fee, fee, mocker, caplog):
    ...
    order_obj = Order.parse_from_ccxt_object(buy_order_fee, "LTC/ETH", "buy")
    res = HuangTrader.get_real_amount(trade, limit_buy_order_usdt, order_obj)
    assert res is None
    ...
    caplog.clear()
    HuangTrader.config["exchange"]["unknown_fee_rate"] = 1
    res = HuangTrader.get_real_amount(trade, limit_buy_order_usdt, order_obj)
    assert res is None
...
def test_apply_fee_conditional(
    ...
    walletmock = mocker.patch("HuangTrader.wallets.Wallets.update")
    mocker.patch("HuangTrader.wallets.Wallets.get_free", return_value=wallet)
    trade = Trade(
        pair="LTC/ETH",
        amount=amount,
        exchange="binance",
    ...
    walletmock.reset_mock()
    # Amount is kept as is
    assert HuangTrader.apply_fee_conditional(trade, "LTC", amount, fee_abs, order) == amount_exp
    assert walletmock.call_count == 1
...
def test_apply_fee_conditional_multibuy(
    ...
    assert HuangTrader.apply_fee_conditional(trade, "LTC", amount, fee_abs, order1) == amount_exp
    assert walletmock.call_count == 1
    if fee_abs != 0 and amount_exp is None:
        assert log_has_re(r"Fee amount.*Eating.*dust\.", caplog)
...
def test_order_book_entry_pricing1(
    ...
    if exception_thrown:
        with pytest.raises(PricingError):
            HuangTrader.exchange.get_rate("ETH/USDT", side="entry", is_short=False, refresh=True)
        assert log_has_re(
...
def test_check_depth_of_market(default_conf_usdt, mocker, order_book_l2) -> None:
    ...
    assert HuangTrader._check_depth_of_market("ETH/BTC", conf, side=SignalDirection.LONG) is False
...
def test_startup_state(default_conf_usdt, mocker):
    ...
    assert worker.HuangTrader.state is State.RUNNING


def test_startup_trade_reinit(default_conf_usdt, edge_conf, mocker):
    mocker.patch(f"{EXMS}.exchange_has", MagicMock(return_value=True))
    reinit_mock = MagicMock()
    mocker.patch("HuangTrader.persistence.Trade.stoploss_reinitialization", reinit_mock)
    ...
    ftbot = get_patched_HuangTraderbot(mocker, edge_conf)
    ftbot.startup()
...
def test_sync_wallet_dry_run(
    ...
    bot = get_patched_HuangTraderbot(mocker, default_conf_usdt)
...
def test_cancel_all_open_orders(
    ...
    buy_mock = mocker.patch("HuangTrader.HuangTraderbot.HuangTraderBot.handle_cancel_enter")
    sell_mock = mocker.patch("HuangTrader.HuangTraderbot.HuangTraderBot.handle_cancel_exit")

    HuangTrader = get_patched_HuangTraderbot(mocker, default_conf_usdt)
    create_mock_trades(fee, is_short=is_short)
    trades = Trade.session.scalars(select(Trade)).all()
    assert len(trades) == MOCK_TRADE_COUNT
    HuangTrader.cancel_all_open_orders()
...
def test_check_for_open_trades(mocker, default_conf_usdt, fee, is_short):
    ...
    HuangTrader.check_for_open_trades()
    assert HuangTrader.rpc.send_msg.call_count == 0
    ...
    HuangTrader.check_for_open_trades()
    assert HuangTrader.rpc.send_msg.call_count == 1
    assert "Handle these trades manually" in HuangTrader.rpc.send_msg.call_args[0][0]["status"]
...
def test_startup_update_open_orders(mocker, default_conf_usdt, fee, caplog, is_short):
    ...
    HuangTrader.startup_update_open_orders()
    assert not log_has_re(r"Error updating Order .*", caplog)
    caplog.clear()

    HuangTrader.config["dry_run"] = False
    HuangTrader.startup_update_open_orders()
    ...
    hto_mock = mocker.patch("HuangTrader.HuangTraderbot.HuangTraderBot.handle_cancel_order")
...
def test_startup_backpopulate_precision(mocker, default_conf_usdt, fee, caplog):
    ...
    HuangTrader.startup_backpopulate_precision()
    trades = Trade.get_trades().all()
    for trade in trades:
...
def test_update_trades_without_assigned_fees(mocker, default_conf_usdt, fee, is_short):
    ...
    HuangTrader.update_trades_without_assigned_fees()
...
def test_reupdate_enter_order_fees(mocker, default_conf_usdt, fee, caplog, is_short):
    HuangTrader = get_patched_HuangTraderbot(mocker, default_conf_usdt)
    mock_uts = mocker.patch("HuangTrader.HuangTraderbot.HuangTraderBot.update_trade_state")

    mocker.patch(f"{EXMS}.fetch_order_or_stoploss_order", return_value={"status": "open"})
    create_mock_trades(fee, is_short)
    trades = Trade.get_trades().all()

    HuangTrader.handle_insufficient_funds(trades[3])
    ...
    HuangTrader.handle_insufficient_funds(trade)
    # assert log_has_re(r"Trying to reupdate buy fees for .*", caplog)
...
def test_handle_insufficient_funds(mocker, default_conf_usdt, fee, is_short, caplog):
    ...
    HuangTrader.handle_insufficient_funds(trades[4])
...
def test_handle_onexchange_order(mocker, default_conf_usdt, limit_order, is_short, caplog):
    ...
    HuangTrader = get_patched_HuangTraderbot(mocker, default_conf_usdt)
    mock_uts = mocker.spy(HuangTrader, "update_trade_state")
    ...
    Trade.session.add(trade)
    HuangTrader.handle_onexchange_order(trade)
...
def test_handle_onexchange_order_changed_amount(
    ...
    HuangTrader.wallets = MagicMock()
    HuangTrader.wallets.get_total = MagicMock(return_value=entry_order["amount"] * factor)
...
def test_handle_onexchange_order_fully_canceled_enter(
    ...
    trade.orders.append(Order.parse_from_ccxt_object(entry_order, "ADA/USDT", entry_side(is_short)))
    Trade.session.add(trade)
    assert HuangTrader.handle_onexchange_order(trade) is True
...
def test_get_valid_price(mocker, default_conf_usdt) -> None:
    ...
    patch_exchange(mocker)
    HuangTrader = HuangTraderBot(default_conf_usdt)
    HuangTrader.config["custom_price_max_distance_ratio"] = 0.02
    ...
    valid_price_from_string = HuangTrader.get_valid_price(custom_price_string, proposed_price)
    valid_price_from_badstring = HuangTrader.get_valid_price(custom_price_badstring, proposed_price)
    valid_price_from_int = HuangTrader.get_valid_price(custom_price_int, proposed_price)
    valid_price_from_float = HuangTrader.get_valid_price(custom_price_float, proposed_price)

    valid_price_at_max_alwd = HuangTrader.get_valid_price(custom_price_over_max_alwd, proposed_price)
    valid_price_at_min_alwd = HuangTrader.get_valid_price(custom_price_under_min_alwd, proposed_price)
...
def test_update_funding_fees_schedule(
    ...
    patch_RPCManager(mocker)
    patch_exchange(mocker)
    mocker.patch("HuangTrader.HuangTraderbot.HuangTraderBot.update_funding_fees", return_value=True)
    default_conf["trading_mode"] = trading_mode
    default_conf["margin_mode"] = "isolated"
    HuangTrader = get_patched_HuangTraderbot(mocker, default_conf)

    time_machine.move_to(f"{t2} {tzoffset}", tick=False)
    # Check schedule jobs in debugging with HuangTrader._schedule.jobs
    HuangTrader._schedule.run_pending()

    assert HuangTrader.update_funding_fees.call_count == calls
...
def test_update_funding_fees(
    ...
    HuangTrader.execute_entry("ETH/USDT", 123, is_short=is_short)
    HuangTrader.execute_entry("LTC/USDT", 2.0, is_short=is_short)
    HuangTrader.execute_entry("XRP/USDT", 123, is_short=is_short)
...
def test_update_funding_fees_error(mocker, default_conf, caplog):
    ...
    default_conf["margin_mode"] = "isolated"
    HuangTrader = get_patched_HuangTraderbot(mocker, default_conf)
    HuangTrader.update_funding_fees()
...
def test_position_adjust(mocker, default_conf_usdt, fee) -> None:
    ...
    assert HuangTrader.execute_entry(pair, stake_amount, trade=trade)
    ...
    assert HuangTrader.execute_trade_exit(
        trade=trade,
        limit=8,
...
def test_position_adjust2(mocker, default_conf_usdt, fee) -> None:
    ...
    assert HuangTrader.execute_entry(pair, amount)
    # Should create an closed trade with an no open order id
    # Order is filled and trade is open
    orders = Order.session.scalars(select(Order)).all()
    assert orders
...
def test_position_adjust3(mocker, default_conf_usdt, fee, data) -> None:
    ...
    for idx, (order, result) in enumerate(data):
        ...
        if order[0] == "buy":
            assert HuangTrader.execute_entry(pair, amount, trade=trade)
        else:
            assert HuangTrader.execute_trade_exit(
                trade=trade,
                limit=price,
...
def test_process_open_trade_positions_exception(mocker, default_conf_usdt, fee, caplog) -> None:
    ...
    mocker.patch(
        "HuangTrader.HuangTraderbot.HuangTraderBot.check_and_call_adjust_trade_position",
        side_effect=DependencyException(),
    )

    create_mock_trades(fee)

    HuangTrader.process_open_trade_positions()
...
def test_check_and_call_adjust_trade_position(mocker, default_conf_usdt, fee, caplog) -> None:
    ...
    create_mock_trades(fee)
    caplog.set_level(logging.DEBUG)
    HuangTrader.strategy.adjust_trade_position = MagicMock(return_value=(10, "aaaa"))
    HuangTrader.process_open_trade_positions()
    assert log_has_re(r"Max adjustment entries for .* has been reached\.", caplog)
    assert HuangTrader.strategy.adjust_trade_position.call_count == 1

    caplog.clear()
    HuangTrader.strategy.adjust_trade_position = MagicMock(return_value=(-0.0005, "partial_exit_c"))
    HuangTrader.process_open_trade_positions()
