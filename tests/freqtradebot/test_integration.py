...
from HuangTrader.enums import ExitCheckTuple, ExitType, TradingMode
from HuangTrader.persistence import Trade
from HuangTrader.persistence.models import Order
from HuangTrader.rpc.rpc import RPC
from tests.conftest import EXMS, get_patched_HuangTraderbot, log_has_re, patch_get_signal
...
def test_may_execute_exit_stoploss_on_exchange_multi(default_conf, ticker, fee, mocker) -> None:
    ...
    * 3rd trade is sold via sell-signal
    ...
    mocker.patch("HuangTrader.strategy.interface.IStrategy.should_exit", should_sell_mock)
    wallets_mock = mocker.patch("HuangTrader.wallets.Wallets.update")
    mocker.patch("HuangTrader.wallets.Wallets.get_free", return_value=1000)
    mocker.patch("HuangTrader.wallets.Wallets.check_exit_amount", return_value=True)

    HuangTrader = get_patched_HuangTraderbot(mocker, default_conf)
    HuangTrader.strategy.order_types["stoploss_on_exchange"] = True
    # Switch ordertype to market to close trade immediately
    HuangTrader.strategy.order_types["exit"] = "market"
    ...
    assert HuangTrader.strategy.confirm_trade_entry.call_count == 3
    HuangTrader.strategy.confirm_trade_entry.reset_mock()
    ...
    assert HuangTrader.strategy.confirm_trade_entry.call_count == 0
    assert HuangTrader.strategy.confirm_trade_exit.call_count == 1
    HuangTrader.strategy.confirm_trade_exit.reset_mock()
...
def test_forcebuy_last_unlimited(default_conf, ticker, fee, mocker, balance_ratio, result1) -> None:
    ...
    trades = Trade.session.scalars(select(Trade)).all()
    assert len(trades) == 4
    assert HuangTrader.wallets.get_trade_stake_amount("XRP/BTC", 5) == result1
    ...
    assert len(trades) == 5
    bals = HuangTrader.wallets.get_all_balances()
    ...
    bals2 = HuangTrader.wallets.get_all_balances()
    assert bals != bals2
    assert len(bals) == 6
    assert len(bals2) == 5
    assert "LTC" in bals
...
def test_dca_short(default_conf_usdt, ticker_usdt, fee, mocker) -> None:
    ...
    patch_get_signal(HuangTrader, enter_long=False, enter_short=True)
    HuangTrader.enter_positions()
...
def test_dca_order_adjust(default_conf_usdt, ticker_usdt, leverage, fee, mocker) -> None:
    ...
    patch_get_signal(HuangTrader)
    HuangTrader.strategy.custom_entry_price = lambda **kwargs: ticker_usdt["ask"] * 0.96
    HuangTrader.strategy.leverage = MagicMock(return_value=leverage)
    HuangTrader.strategy.minimal_roi = {0: 0.2}
    ...
    HuangTrader.strategy.adjust_entry_price = MagicMock(return_value=1.99)
    HuangTrader.process()
    trade = Trade.get_trades().first()
    assert len(trade.orders) == 2
    assert trade.has_open_orders
    ...
    HuangTrader.strategy.adjust_trade_position = MagicMock(return_value=120)
    mocker.patch(f"{EXMS}._dry_is_price_crossed", return_value=False)
    ...
    HuangTrader.strategy.adjust_entry_price = MagicMock(return_value=1.95)
    ...
    # Fill DCA order
    HuangTrader.strategy.adjust_trade_position = MagicMock(return_value=None)
    mocker.patch(f"{EXMS}._dry_is_price_crossed", return_value=True)
    HuangTrader.strategy.adjust_entry_price = MagicMock(side_effect=ValueError)
    ...
    # Full exit
    mocker.patch(f"{EXMS}._dry_is_price_crossed", return_value=False)
    HuangTrader.strategy.custom_exit = MagicMock(return_value="Exit now")
    HuangTrader.strategy.adjust_entry_price = MagicMock(return_value=2.02)
    HuangTrader.process()
    trade = Trade.get_trades().first()
...
def test_dca_order_adjust_entry_replace_fails(
    ...
    HuangTrader.strategy.adjust_trade_position = MagicMock(return_value=(20, "PeNF"))
    ...
    HuangTrader.strategy.adjust_entry_price = MagicMock(return_value=2.05)
    ...
    assert HuangTrader.strategy.adjust_entry_price.call_count == 2
    ...
    entry_mock = mocker.patch(
        "HuangTrader.HuangTraderbot.HuangTraderBot.execute_entry", return_value=False
    )
...
def test_dca_exiting(default_conf_usdt, ticker_usdt, fee, mocker, caplog, leverage) -> None:
    ...
    if not spot:
        ...
        default_conf_usdt["margin_mode"] = "isolated"
    HuangTrader = get_patched_HuangTraderbot(mocker, default_conf_usdt)
    assert HuangTrader.trading_mode == TradingMode.FUTURES if not spot else TradingMode.SPOT
    mocker.patch.multiple(
        EXMS,
        fetch_ticker=ticker_usdt,
        get_fee=fee,
    ...
    mocker.patch(f"{EXMS}.get_max_leverage", return_value=10)
    starting_amount = HuangTrader.wallets.get_total("USDT")
    ...
    assert trade.open_rate == 2.0
    assert pytest.approx(HuangTrader.wallets.get_free("USDT")) == starting_amount - 60
    if spot:
        assert pytest.approx(HuangTrader.wallets.get_total("USDT")) == starting_amount - 60
    else:
        assert HuangTrader.wallets.get_total("USDT") == starting_amount

    # Too small size
    HuangTrader.strategy.adjust_trade_position = MagicMock(return_value=-59)
    HuangTrader.process()
    trade = Trade.get_trades().first()
    assert len(trade.orders) == 1
    assert pytest.approx(trade.stake_amount) == 60
    ...
    HuangTrader.strategy.adjust_trade_position = MagicMock(return_value=(-20, "PES"))
    ...
    assert pytest.approx(HuangTrader.wallets.get_free("USDT")) == expected_profit

    if spot:
        assert pytest.approx(HuangTrader.wallets.get_total("USDT")) == expected_profit
    else:
        # total won't change in futures mode, only free / used will.
        assert HuangTrader.wallets.get_total("USDT") == starting_amount + trade.realized_profit
    caplog.clear()

    # Sell more than what we got (we got ~20 coins left)
    # Doesn't exit, as the amount is too high.
    HuangTrader.strategy.adjust_trade_position = MagicMock(return_value=-50)
    HuangTrader.process()
    trade = Trade.get_trades().first()
    assert len(trade.orders) == 2

    # Amount too low...
    HuangTrader.strategy.adjust_trade_position = MagicMock(return_value=-(trade.stake_amount * 0.99))
    HuangTrader.process()
    ...
    HuangTrader.strategy.adjust_trade_position = MagicMock(return_value=-trade.stake_amount)
    HuangTrader.process()
    ...
    HuangTrader.strategy.adjust_trade_position = MagicMock(return_value=-0.01)
    HuangTrader.process()
    trade = Trade.get_trades().first()
    assert len(trade.orders) == 3
    assert trade.orders[-1].ft_order_side == "sell"
