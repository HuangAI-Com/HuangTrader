...
from HuangTrader.enums import ExitCheckTuple, ExitType, RPCMessageType
from HuangTrader.exceptions import ExchangeError, InsufficientFundsError, InvalidOrderException
from HuangTrader.HuangTraderbot import HuangTraderBot
from HuangTrader.persistence import Order, Trade
...
from tests.HuangTraderbot.test_HuangTraderbot import patch_RPCManager
...
def test_add_stoploss_on_exchange(mocker, default_conf_usdt, limit_order, is_short, fee) -> None:
    ...
    HuangTrader.exit_positions(trades)
...
def test_handle_stoploss_on_exchange(
    ...
    assert HuangTrader.handle_stoploss_on_exchange(trade) is False
    assert stoploss.call_count == 1
    ...
    assert HuangTrader.handle_stoploss_on_exchange(trade) is True
    ...
    trade.is_open = True
    HuangTrader.handle_stoploss_on_exchange(trade)
...
def test_handle_stoploss_on_exchange_emergency(
    ...
    HuangTrader.config["trailing_stop"] = True
...
def test_handle_stoploss_on_exchange_partial_cancel_here(
    ...
    # HuangTrader cancels the stop - but cancel returns a partial filled order.
    stoploss_order_hit = MagicMock(
        return_value={
            "id": "101",
            "status": "open",
...
def test_create_stoploss_order_invalid_order(
    ...
    caplog.clear()
    rpc_mock.reset_mock()
    HuangTrader.create_stoploss_order(trade, 200)
    assert trade.has_open_sl_orders is False
...
def test_handle_stoploss_on_exchange_trailing(
    ...
    # setting stoploss
    HuangTrader.strategy.stoploss = 0.05 if is_short else -0.05

    # setting stoploss_on_exchange_interval to 60 seconds
    HuangTrader.strategy.order_types["stoploss_on_exchange_interval"] = 60
    ...
    stoploss_order_mock.assert_called_once_with(
        amount=30,
        pair="ETH/USDT",
        order_types=HuangTrader.strategy.order_types,
...
def test_handle_stoploss_on_exchange_trailing_error(
    ...
    HuangTrader.handle_trailing_stoploss_on_exchange(trade, stoploss_order_hanging)
...
def test_stoploss_on_exchange_price_rounding(
    ...
    HuangTrader.handle_trailing_stoploss_on_exchange(open_trade_usdt, {})
...
def test_handle_stoploss_on_exchange_custom_stop(
    ...
    # setting stoploss
    HuangTrader.strategy.custom_stoploss = lambda *args, **kwargs: -0.04
    ...
    HuangTrader.strategy.order_types["stoploss_on_exchange_interval"] = 0
    cancel_order_mock.assert_not_called()
    stoploss_order_mock.assert_not_called()
...
def test_tsl_on_exchange_compatible_with_edge(mocker, edge_conf, fee, limit_order) -> None:
    ...
    # setting stoploss
    HuangTrader.strategy.stoploss = -0.02
    ...
    HuangTrader.active_pair_whitelist = HuangTrader.edge.adjust(HuangTrader.active_pair_whitelist)
...
def test_execute_trade_exit_sloe_cancel_exception(
    ...
    HuangTrader = get_patched_HuangTraderbot(mocker, default_conf_usdt)
    mocker.patch(f"{EXMS}.cancel_stoploss_order", side_effect=InvalidOrderException())
    mocker.patch("HuangTrader.wallets.Wallets.get_free", MagicMock(return_value=300))
    create_order_mock = MagicMock(
        side_effect=[
            {"id": "12345554"},
            {"id": "12345555"},
...
def test_may_execute_trade_exit_after_stoploss_on_exchange_hit(
    ...
    HuangTrader = HuangTraderBot(default_conf_usdt)
    HuangTrader.strategy.order_types["stoploss_on_exchange"] = True
    patch_get_signal(HuangTrader, enter_long=not is_short, enter_short=is_short)
