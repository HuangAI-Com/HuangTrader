...
def test_worker_state(mocker, default_conf, markets) -> None:
    ...
    assert worker.HuangTrader.state is State.STOPPED


def test_worker_running(mocker, default_conf, caplog) -> None:
    mock_throttle = MagicMock()
    mocker.patch("HuangTrader.worker.Worker._throttle", mock_throttle)
    mocker.patch("HuangTrader.persistence.Trade.stoploss_reinitialization", MagicMock())
    ...
    assert worker.HuangTrader.strategy
    assert worker.HuangTrader.strategy.dp
    assert isinstance(worker.HuangTrader.strategy.dp, DataProvider)
...
def test_worker_stopped(mocker, default_conf, caplog) -> None:
    ...
    worker = get_patched_worker(mocker, default_conf)
    worker.HuangTrader.state = State.STOPPED
...
def test_throttle_sleep_time(mocker, default_conf, caplog) -> None:
    caplog.set_level(logging.DEBUG)
    worker = get_patched_worker(mocker, default_conf)
    sleep_mock = mocker.patch("HuangTrader.worker.Worker._sleep")
...
def test_worker_heartbeat_running(default_conf, mocker, caplog):
    ...
    worker.HuangTrader.state = State.RUNNING
