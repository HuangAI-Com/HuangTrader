...
from HuangTrader.commands.optimize_commands import setup_optimize_configuration, start_backtesting
from HuangTrader.configuration import TimeRange
from HuangTrader.data import history
from HuangTrader.data.btanalysis import BT_DATA_COLUMNS, evaluate_result_multi
from HuangTrader.data.converter import clean_ohlcv_dataframe
from HuangTrader.data.dataprovider import DataProvider
from HuangTrader.data.history import get_timerange
from HuangTrader.enums import CandleType, ExitType, RunMode
from HuangTrader.exceptions import DependencyException, OperationalException
from HuangTrader.exchange import timeframe_to_next_date, timeframe_to_prev_date
from HuangTrader.optimize.backtest_caching import get_backtest_metadata_filename, get_strategy_run_id
...
def test_setup_bt_configuration_with_arguments(mocker, default_conf, caplog) -> None:
    patched_configuration_load_config_file(mocker, default_conf)
    mocker.patch("HuangTrader.configuration.configuration.create_datadir", lambda c, x: x)
...
def test_start(mocker, fee, default_conf, caplog) -> None:
    ...
    patch_exchange(mocker)
    mocker.patch("HuangTrader.optimize.backtesting.Backtesting.start", start_mock)
    patched_configuration_load_config_file(mocker, default_conf)
    ...
    assert log_has("Starting HuangTrader in Backtesting mode", caplog)
...
def test_backtesting_start(default_conf, mocker, caplog) -> None:
    ...
    mocker.patch("HuangTrader.data.history.get_timerange", get_timerange)
    patch_exchange(mocker)
    mocker.patch("HuangTrader.optimize.backtesting.Backtesting.backtest")
    mocker.patch("HuangTrader.optimize.backtesting.generate_backtest_stats")
    mocker.patch("HuangTrader.optimize.backtesting.show_backtest_results")
    sbs = mocker.patch("HuangTrader.optimize.backtesting.store_backtest_stats")
    sbc = mocker.patch("HuangTrader.optimize.backtesting.store_backtest_analysis_results")
    mocker.patch(
        "HuangTrader.plugins.pairlistmanager.PairListManager.whitelist",
...
def test_backtesting_start_no_data(default_conf, mocker, caplog, testdatadir) -> None:
    ...
    mocker.patch(
        "HuangTrader.data.history.history_utils.load_pair_history",
...
def test_backtesting_no_pair_left(default_conf, mocker, caplog, testdatadir) -> None:
    ...
    mocker.patch("HuangTrader.optimize.backtesting.Backtesting.backtest")
    mocker.patch(
        "HuangTrader.plugins.pairlistmanager.PairListManager.whitelist", PropertyMock(return_value=[])
    )
...
def test_backtesting_pairlist_list(default_conf, mocker, caplog, testdatadir, tickers) -> None:
    ...
    mocker.patch("HuangTrader.plugins.pairlistmanager.PairListManager.refresh_pairlist")
...
def test_backtest__enter_trade_futures(default_conf_usdt, fee, mocker) -> None:
    ...
    mocker.patch("HuangTrader.optimize.backtesting.price_to_precision", lambda p, *args: p)
    patch_exchange(mocker)
    ...
    # HuangTrader_liquidation_price = liq + (abs(open_rate - liq) * liq_buffer * side_1)
    ...
    mocker.patch(
        "HuangTrader.wallets.Wallets.get_trade_stake_amount", side_effect=DependencyException
    )
...
def test_backtest_start_multi_strat(default_conf, mocker, caplog, testdatadir):
    ...
    mocker.patch("HuangTrader.optimize.backtesting.Backtesting.backtest", backtestmock)
    ...
    mocker.patch.multiple(
        "HuangTrader.optimize.optimize_reports.bt_output",
        text_table_bt_results=text_table_mock,
        text_table_strategy=strattable_mock,
    )
    mocker.patch.multiple(
        "HuangTrader.optimize.optimize_reports.optimize_reports",
...
def test_backtest_start_multi_strat_caching(
    ...
    mocker.patch("HuangTrader.optimize.backtesting.Backtesting.backtest", backtestmock)
    mocker.patch("HuangTrader.optimize.backtesting.show_backtest_results", MagicMock())
    ...
    mocker.patch.multiple(
        "HuangTrader.data.btanalysis",
        load_backtest_metadata=load_backtest_metadata,
        load_backtest_stats=load_backtest_stats,
    )
    mocker.patch("HuangTrader.optimize.backtesting.get_strategy_run_id", side_effect=["1", "2", "2"])
