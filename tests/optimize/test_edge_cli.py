...
from HuangTrader.commands.optimize_commands import setup_optimize_configuration, start_edge
...
def test_start(mocker, fee, edge_conf, caplog) -> None:
    ...
    mocker.patch("HuangTrader.optimize.edge_cli.EdgeCli.start", start_mock)
    patched_configuration_load_config_file(mocker, edge_conf)
    ...
    pargs = get_args(args)
    start_edge(pargs)
    assert log_has("Starting HuangTrader in Edge mode", caplog)
...
def test_edge_start(mocker, edge_conf) -> None:
    mock_calculate = mocker.patch(
        "HuangTrader.edge.edge_positioning.Edge.calculate", return_value=True
    )
    table_mock = mocker.patch("HuangTrader.optimize.edge_cli.generate_edge_table")
