...
from HuangTrader.commands.optimize_commands import setup_optimize_configuration, start_hyperopt
from HuangTrader.data.history import load_data
from HuangTrader.enums import ExitType, RunMode
from HuangTrader.exceptions import OperationalException
from HuangTrader.optimize.hyperopt import Hyperopt
from HuangTrader.optimize.hyperopt_auto import HyperOptAuto
from HuangTrader.optimize.hyperopt_tools import HyperoptTools
from HuangTrader.optimize.optimize_reports import generate_strategy_stats
from HuangTrader.optimize.space import SKDecimal
from HuangTrader.strategy import IntParameter
from HuangTrader.util import dt_utc
...
def test_start_not_installed(mocker, default_conf, import_fails) -> None:
    ...
    mocker.patch("HuangTrader.optimize.hyperopt.Hyperopt.start", start_mock)
...
def test_start_no_data(mocker, hyperopt_conf, tmp_path) -> None:
    ...
    patched_configuration_load_config_file(mocker, hyperopt_conf)
    mocker.patch("HuangTrader.data.history.load_pair_history", MagicMock(return_value=pd.DataFrame))
    mocker.patch(
        "HuangTrader.optimize.hyperopt.get_timerange",
...
def test_start_filelock(mocker, hyperopt_conf, caplog) -> None:
    ...
    patched_configuration_load_config_file(mocker, hyperopt_conf)
    mocker.patch("HuangTrader.optimize.hyperopt.Hyperopt.__init__", hyperopt_mock)
    patch_exchange(mocker)
    ...
    pargs = get_args(args)
    start_hyperopt(pargs)
    assert log_has("Another running instance of HuangTrader Hyperopt detected.", caplog)
...
def test_start_calls_optimizer(mocker, hyperopt_conf, capsys) -> None:
    dumper = mocker.patch("HuangTrader.optimize.hyperopt.dump")
    dumper2 = mocker.patch("HuangTrader.optimize.hyperopt.Hyperopt._save_result")
    mocker.patch("HuangTrader.optimize.hyperopt.calculate_market_change", return_value=1.5)
    mocker.patch("HuangTrader.optimize.hyperopt.file_dump_json")

    mocker.patch(
        "HuangTrader.optimize.backtesting.Backtesting.load_bt_data",
    ...
    mocker.patch("HuangTrader.optimize.hyperopt.INITIAL_POINTS", 2)

    parallel = mocker.patch(
        "HuangTrader.optimize.hyperopt.Hyperopt.run_optimizer_parallel",
        MagicMock(
            return_value=[
                {
                    "loss": 1,
...
def test_generate_optimizer(mocker, hyperopt_conf) -> None:
    ...
    mocker.patch("HuangTrader.optimize.hyperopt.Backtesting.backtest", return_value=backtest_result)
    mocker.patch(
        "HuangTrader.optimize.hyperopt.get_timerange",
    ...
    mocker.patch("HuangTrader.configuration.config_validation.validate_config_schema")
    mocker.patch("HuangTrader.optimize.hyperopt.load", return_value={"XRP/BTC": None})
...
def test_clean_hyperopt(mocker, hyperopt_conf, caplog):
    ...
    mocker.patch(
        "HuangTrader.strategy.hyper.HyperStrategyMixin.load_params_from_file",
        MagicMock(return_value={}),
    )
    mocker.patch("HuangTrader.optimize.hyperopt.Path.is_file", MagicMock(return_value=True))
    unlinkmock = mocker.patch("HuangTrader.optimize.hyperopt.Path.unlink", MagicMock())
...
def test_simplified_interface_all_failed(mocker, hyperopt_conf, caplog) -> None:
    mocker.patch("HuangTrader.optimize.hyperopt.dump", MagicMock())
    mocker.patch("HuangTrader.optimize.hyperopt.file_dump_json")
    mocker.patch(
        "HuangTrader.optimize.backtesting.Backtesting.load_bt_data",
        MagicMock(return_value=(MagicMock(), None)),
    ...
    mocker.patch(
        "HuangTrader.optimize.hyperopt_auto.HyperOptAuto._generate_indicator_space", return_value=[]
    )
...
def test_in_strategy_auto_hyperopt_per_epoch(mocker, hyperopt_conf, tmp_path, fee) -> None:
    ...
    go = mocker.patch(
        "HuangTrader.optimize.hyperopt.Hyperopt.generate_optimizer",
        return_value={
            "loss": 0.05,
            "results_explanation": "foo result",
            "params": {},
...
def test_stake_amount_unlimited_max_open_trades(mocker, hyperopt_conf, tmp_path, fee) -> None:
    ...
    hyperopt = Hyperopt(hyperopt_conf)
    mocker.patch(
        "HuangTrader.optimize.hyperopt.Hyperopt._get_params_dict",
