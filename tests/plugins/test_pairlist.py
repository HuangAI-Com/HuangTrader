...
from HuangTrader.constants import AVAILABLE_PAIRLISTS
from HuangTrader.data.dataprovider import DataProvider
from HuangTrader.enums import CandleType, RunMode
from HuangTrader.exceptions import OperationalException
from HuangTrader.persistence import LocalTrade, Trade
from HuangTrader.plugins.pairlist.pairlist_helpers import dynamic_expand_pairlist, expand_pairlist
from HuangTrader.plugins.pairlistmanager import PairListManager
from HuangTrader.resolvers import PairListResolver
...
def test_log_cached(mocker, static_pl_conf, markets, tickers):
    ...
    HuangTrader = get_patched_HuangTraderbot(mocker, static_pl_conf)
    logmock = MagicMock()
    # Assign starting whitelist
    pl = HuangTrader.pairlists._pairlist_handlers[0]
...
def test_load_pairlist_noexist(mocker, markets, default_conf):
    ...
    plm = PairListManager(HuangTrader.exchange, default_conf, MagicMock())
    with pytest.raises(
        ...
        PairListResolver.load_pairlist(
            "NonexistingPairList", HuangTrader.exchange, plm, default_conf, {}, 1
        )
...
def test_refresh_market_pair_not_in_whitelist(mocker, markets, static_pl_conf):
    ...
    mocker.patch(f"{EXMS}.markets", PropertyMock(return_value=markets))
    HuangTrader.pairlists.refresh_pairlist()
    # List ordered by BaseVolume
    whitelist = ["ETH/BTC", "TKN/BTC"]
    # Ensure all except those in whitelist are removed
    assert set(whitelist) == set(HuangTrader.pairlists.whitelist)
    # Ensure config dict hasn't been changed
    assert (
        static_pl_conf["exchange"]["pair_whitelist"]
        == HuangTrader.config["exchange"]["pair_whitelist"]
    )
...
def test_refresh_static_pairlist(mocker, markets, static_pl_conf):
    ...
    # Ensure all except those in whitelist are removed
    assert set(whitelist) == set(HuangTrader.pairlists.whitelist)
    assert static_pl_conf["exchange"]["pair_blacklist"] == HuangTrader.pairlists.blacklist
...
def test_refresh_static_pairlist_noexist(mocker, markets, static_pl_conf, pairs, expected, caplog):
    ...
    # Ensure all except those in whitelist are removed
    assert set(expected) == set(HuangTrader.pairlists.whitelist)
    assert static_pl_conf["exchange"]["pair_blacklist"] == HuangTrader.pairlists.blacklist
    if not expected:
...
def test_remove_logs_for_pairs_already_in_blacklist(mocker, markets, static_pl_conf, caplog):
    ...
    for _ in range(3):
        new_whitelist = HuangTrader.pairlists.verify_blacklist(
...
def test_refresh_pairlist_dynamic(mocker, shitcoinmarkets, tickers, whitelist_conf):
    ...
    HuangTrader = get_patched_HuangTraderbot(mocker, whitelist_conf)
    # Remock markets with shitcoinmarkets since get_patched_HuangTraderbot uses the markets fixture
    mocker.patch.multiple(
        EXMS,
    ...
    HuangTrader.pairlists.refresh_pairlist()
    assert whitelist == HuangTrader.pairlists.whitelist
    ...
    with pytest.raises(
        ...
        PairListManager(HuangTrader.exchange, whitelist_conf, MagicMock())
...
def test_refresh_pairlist_dynamic_2(mocker, shitcoinmarkets, tickers, whitelist_conf_2):
    ...
    mocker.patch.multiple(
        "HuangTrader.plugins.pairlistmanager.PairListManager",
        _get_cached_tickers=MagicMock(return_value=tickers_dict),
    )
    HuangTrader = get_patched_HuangTraderbot(mocker, whitelist_conf_2)
    # Remock markets with shitcoinmarkets since get_patched_HuangTraderbot uses the markets fixture
    mocker.patch.multiple(
        EXMS,
        markets=PropertyMock(return_value=shitcoinmarkets),
...
def test_VolumePairList_whitelist_gen(
    ...
    mocker.patch.multiple(
        "HuangTrader.persistence.Trade", get_overall_performance=MagicMock(return_value=[])
    )
    ...
    if whitelist_result == "filter_at_the_beginning":
        ...
        HuangTrader.pairlists.refresh_pairlist()
        whitelist = HuangTrader.pairlists.whitelist
...
def test_gen_pair_whitelist_not_supported(mocker, default_conf, tickers) -> None:
    ...
    with pytest.raises(
        ...
        get_patched_HuangTraderbot(mocker, default_conf)
...
def test_pairlist_class(mocker, whitelist_conf, markets, pairlist):
    ...
    assert HuangTrader.pairlists.name_list == [pairlist]
    assert pairlist in str(HuangTrader.pairlists.short_desc())
    assert isinstance(HuangTrader.pairlists.whitelist, list)
    assert isinstance(HuangTrader.pairlists.blacklist, list)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["ETH/BTC", "TKN/BTC", "XLTCUSDT"], "is not tradable with HuangTrader"),
    ],
)
def test__whitelist_for_active_markets(
    ...
    # Assign starting whitelist
    pairlist_handler = HuangTrader.pairlists._pairlist_handlers[0]
...
def test_volumepairlist_invalid_sortvalue(mocker, whitelist_conf):
    ...
    with pytest.raises(OperationalException, match=r"key asdf not in .*"):
        get_patched_HuangTraderbot(mocker, whitelist_conf)
...
def test_volumepairlist_caching(mocker, markets, whitelist_conf, tickers):
    ...
    HuangTrader = get_patched_HuangTraderbot(mocker, whitelist_conf)
    assert len(HuangTrader.pairlists._pairlist_handlers[0]._pair_cache) == 0
    assert tickers.call_count == 0
    HuangTrader.pairlists.refresh_pairlist()
    assert tickers.call_count == 1

    assert len(HuangTrader.pairlists._pairlist_handlers[0]._pair_cache) == 1
    HuangTrader.pairlists.refresh_pairlist()
    assert tickers.call_count == 1
...
def test_agefilter_caching(mocker, markets, whitelist_conf_agefilter, tickers, ohlcv_history):
    with time_machine.travel("2021-09-01 05:00:00 +00:00") as t:
        ...
        HuangTrader = get_patched_HuangTraderbot(mocker, whitelist_conf_agefilter)
        assert HuangTrader.exchange.refresh_latest_ohlcv.call_count == 0
        HuangTrader.pairlists.refresh_pairlist()
        assert len(HuangTrader.pairlists.whitelist) == 3
        assert HuangTrader.exchange.refresh_latest_ohlcv.call_count > 0

        HuangTrader.pairlists.refresh_pairlist()
        assert len(HuangTrader.pairlists.whitelist) == 3
        # Call to XRP/BTC cached
        assert HuangTrader.exchange.refresh_latest_ohlcv.call_count == 2
        ...
        HuangTrader.pairlists.refresh_pairlist()
        assert len(HuangTrader.pairlists.whitelist) == 3
        assert HuangTrader.exchange.refresh_latest_ohlcv.call_count == 1
        ...
        mocker.patch(f"{EXMS}.refresh_latest_ohlcv", return_value=ohlcv_data)
        HuangTrader.pairlists.refresh_pairlist()
        assert len(HuangTrader.pairlists.whitelist) == 4
...
def test_rangestabilityfilter_caching(
    ...
    assert HuangTrader.exchange.refresh_latest_ohlcv.call_count == 0
    HuangTrader.pairlists.refresh_pairlist()
    assert len(HuangTrader.pairlists.whitelist) == expected_length
    assert HuangTrader.exchange.refresh_latest_ohlcv.call_count > 0

    previous_call_count = HuangTrader.exchange.refresh_latest_ohlcv.call_count
    HuangTrader.pairlists.refresh_pairlist()
    assert len(HuangTrader.pairlists.whitelist) == expected_length
    # Should not have increased since first call.
    assert HuangTrader.exchange.refresh_latest_ohlcv.call_count == previous_call_count
...
def test_pricefilter_desc(
    ...
    if desc_expected is not None:
        HuangTrader = get_patched_HuangTraderbot(mocker, whitelist_conf)
        short_desc = str(HuangTrader.pairlists.short_desc())
...
def test_performance_filter(
    ...
    HuangTrader = get_patched_HuangTraderbot(mocker, allowlist_conf)
    ...
    mocker.patch.multiple(
        "HuangTrader.persistence.Trade",
        get_overall_performance=MagicMock(return_value=overall_performance),
    )
    HuangTrader.pairlists.refresh_pairlist()
    allowlist = HuangTrader.pairlists.whitelist
...
def test_MarketCapPairList_filter(
    ...
    mocker.patch(
        "HuangTrader.plugins.pairlist.MarketCapPairList.FtCoinGeckoApi.get_coins_markets",
