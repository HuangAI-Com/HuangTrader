...
from HuangTrader.persistence.trade_model import Order
from HuangTrader.plugins.protectionmanager import ProtectionManager
from tests.conftest import get_patched_HuangTraderbot, log_has_re
...
def test_protectionmanager(mocker, default_conf):
    ...
    for handler in HuangTrader.protections._protection_handlers:
...
def test_stoploss_guard(mocker, default_conf, fee, caplog, is_short):
    ...
    assert not HuangTrader.protections.global_stop()
    ...
    assert HuangTrader.protections.global_stop()
    ...
    HuangTrader.protections.global_stop(end_time)
...
def test_stoploss_guard_perpair(mocker, default_conf, fee, caplog, only_per_pair, only_per_side):
    ...
    message = r"Trading stopped due to .*"
    pair = "XRP/BTC"
    assert not HuangTrader.protections.stop_per_pair(pair)
    assert not HuangTrader.protections.global_stop()
    assert not log_has_re(message, caplog)
    caplog.clear()
    ...
    # 3 Trades closed - but the 2nd has been closed too long ago.
    assert not HuangTrader.protections.stop_per_pair(pair)
    assert HuangTrader.protections.global_stop() != only_per_pair
    ...
    HuangTrader.protections.stop_per_pair(pair)
    assert HuangTrader.protections.global_stop() != only_per_pair
...
def test_CooldownPeriod(mocker, default_conf, fee, caplog):
    ...
    message = r"Trading stopped due to .*"
    assert not HuangTrader.protections.global_stop()
    assert not HuangTrader.protections.stop_per_pair("XRP/BTC")
    ...
    assert not HuangTrader.protections.global_stop()
    assert HuangTrader.protections.stop_per_pair("XRP/BTC")
    ...
    assert HuangTrader.protections.stop_per_pair("ETH/BTC")
...
def test_LowProfitPairs(mocker, default_conf, fee, caplog, only_per_side):
    ...
    Trade.commit()
    assert HuangTrader.protections.stop_per_pair("XRP/BTC") != only_per_side
    ...
    assert HuangTrader.protections.global_stop() != only_per_side
    assert HuangTrader.protections.stop_per_pair("XRP/BTC") != only_per_side
...
def test_protection_manager_desc(
    ...
    short_desc = str(HuangTrader.protections.short_desc())
