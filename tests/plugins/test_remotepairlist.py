...
from HuangTrader.plugins.pairlist.RemotePairList import RemotePairList
from HuangTrader.plugins.pairlistmanager import PairListManager
from tests.conftest import EXMS, get_patched_exchange, get_patched_HuangTraderbot, log_has
...
def test_gen_pairlist_with_local_file(mocker, rpl_config):
    ...
    mocker.patch("HuangTrader.plugins.pairlist.RemotePairList.open", return_value=mock_file)

    mock_file_path = mocker.patch("HuangTrader.plugins.pairlist.RemotePairList.Path")
    mock_file_path.exists.return_value = True

    jsonparse = json.loads(mock_file.read.return_value)
    mocker.patch("HuangTrader.plugins.pairlist.RemotePairList.rapidjson.load", return_value=jsonparse)
...
def test_fetch_pairlist_mock_response_html(mocker, rpl_config):
    ...
    mocker.patch(
        "HuangTrader.plugins.pairlist.RemotePairList.requests.get", return_value=mock_response
    )
    remote_pairlist = RemotePairList(
        exchange, pairlistmanager, rpl_config, rpl_config["pairlists"][0], 0
    )
...
def test_fetch_pairlist_timeout_keep_last_pairlist(mocker, rpl_config, caplog):
    ...
    mocker.patch(
        "HuangTrader.plugins.pairlist.RemotePairList.requests.get",
...
def test_remote_pairlist_init_no_pairlist_url(mocker, rpl_config):
    ...
    with pytest.raises(
        ...
        get_patched_HuangTraderbot(mocker, rpl_config)
