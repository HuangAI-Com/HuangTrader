...
from HuangTrader.util.coin_gecko import FtCoinGeckoApi
...
def test_fiat_convert_find_price(mocker):
    ...
    mocker.patch(
        "HuangTrader.rpc.fiat_convert.CryptoToFiatConverter._load_cryptomap", return_value=None
    )
    ...
    mocker.patch(
        "HuangTrader.rpc.fiat_convert.CryptoToFiatConverter._find_price", return_value=12345.0
    )
    assert fiat_convert.get_price(crypto_symbol="BTC", fiat_symbol="USD") == 12345.0
    assert fiat_convert.get_price(crypto_symbol="btc", fiat_symbol="usd") == 12345.0

    mocker.patch(
        "HuangTrader.rpc.fiat_convert.CryptoToFiatConverter._find_price", return_value=13000.2
    )
    assert fiat_convert.get_price(crypto_symbol="BTC", fiat_symbol="EUR") == 13000.2


def test_fiat_convert_unsupported_crypto(mocker, caplog):
    mocker.patch("HuangTrader.rpc.fiat_convert.CryptoToFiatConverter._coinlistings", return_value=[])
    fiat_convert = CryptoToFiatConverter({})
...
def test_fiat_convert_get_price(mocker):
    find_price = mocker.patch(
        "HuangTrader.rpc.fiat_convert.CryptoToFiatConverter._find_price", return_value=28000.0
    )
...
def test_convert_amount(mocker):
    mocker.patch("HuangTrader.rpc.fiat_convert.CryptoToFiatConverter.get_price", return_value=12345.0)
