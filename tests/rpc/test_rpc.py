...
from HuangTrader.enums import SignalDirection, State, TradingMode
from HuangTrader.exceptions import ExchangeError, InvalidOrderException, TemporaryError
from HuangTrader.persistence import Order, Trade
from HuangTrader.persistence.key_value_store import set_startup_time
from HuangTrader.rpc import RPC, RPCException
...
def test_rpc_trade_status(default_conf, ticker, fee, mocker) -> None:
    ...
    # Fill open order ...
    HuangTraderbot.manage_open_orders()
    trades = Trade.get_open_trades()
    HuangTraderbot.exit_positions(trades)
...
def test__rpc_timeunit_profit(
    ...
    HuangTraderbot = get_patched_HuangTraderbot(mocker, default_conf_usdt)
...
def test_rpc_delete_trade(mocker, default_conf, fee, markets, caplog, is_short):
    ...
    HuangTraderbot = get_patched_HuangTraderbot(mocker, default_conf)
    HuangTraderbot.strategy.order_types["stoploss_on_exchange"] = True
    create_mock_trades(fee, is_short)
    rpc = RPC(HuangTraderbot)
...
def test_rpc_balance_handle(default_conf_usdt, mocker, tickers):
    ...
    mocker.patch("HuangTrader.rpc.rpc.CryptoToFiatConverter._find_price", return_value=1.2)
    mocker.patch("HuangTrader.rpc.telegram.Telegram", MagicMock())
    mocker.patch.multiple(
        EXMS,
...
def test_rpc_force_exit(default_conf, ticker, fee, mocker) -> None:
    ...
    HuangTraderbot.config["max_open_trades"] = 3
    HuangTraderbot.enter_positions()
...
def test_rpc_force_entry(mocker, default_conf, ticker, fee, limit_buy_order_open) -> None:
    ...
    HuangTraderbot.config["max_open_trades"] = 5
    ...
    HuangTraderbot.strategy.position_adjustment_enable = True
    ...
    pair = "XRP/BTC"
    HuangTraderbot = get_patched_HuangTraderbot(mocker, default_conf)
    HuangTraderbot.config["stake_amount"] = 0
    patch_get_signal(HuangTraderbot)
    rpc = RPC(HuangTraderbot)
    pair = "TKN/BTC"
...
def test_rpc_edge_enabled(mocker, edge_conf) -> None:
    ...
    HuangTraderbot = get_patched_HuangTraderbot(mocker, edge_conf)
