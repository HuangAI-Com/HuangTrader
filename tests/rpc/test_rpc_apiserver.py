...
from HuangTrader.__init__ import __version__
from HuangTrader.enums import CandleType, RunMode, State, TradingMode
from HuangTrader.exceptions import DependencyException, ExchangeError, OperationalException
from HuangTrader.loggers import setup_logging, setup_logging_pre
...
from HuangTrader.rpc.api_server.api_auth import create_token, get_user_from_token
from HuangTrader.rpc.api_server.uvicorn_threaded import UvicornServer
from HuangTrader.rpc.api_server.webserver_bgwork import ApiBG
from HuangTrader.util.datetime_helpers import format_date
...
def botclient(default_conf, mocker):
    ...
    rpc = RPC(ftbot)
    mocker.patch("HuangTrader.rpc.api_server.ApiServer.start_api", MagicMock())
    apiserver = None
    try:
...
def test_api_ui_fallback(botclient, mocker):
    ...
    assert rc.status_code == 200
    assert "`HuangTrader install-ui`" in rc.text
    ...
    assert "`HuangTrader install-ui`" in rc.text or "<!DOCTYPE html>" in rc.text
...
def test_api_ui_version(botclient, mocker):
    ...
    mocker.patch("HuangTrader.commands.deploy_commands.read_ui_version", return_value="0.1.2")
...
def test_api_auth():
    with pytest.raises(ValueError):
        create_token({"identity": {"u": "HuangTrader"}}, "secret1234", token_type="NotATokenType")

    token = create_token({"identity": {"u": "HuangTrader"}}, "secret1234")
    assert isinstance(token, str)

    u = get_user_from_token(token, "secret1234")
    assert u == "HuangTrader"
...
def test_api_ws_auth(botclient):
    ...
    jwt_token = create_token({"identity": {"u": "HuangTrader"}}, jwt_secret)
...
def test_api__init__(default_conf, mocker):
    ...
    mocker.patch("HuangTrader.rpc.telegram.Telegram._init")
    mocker.patch("HuangTrader.rpc.api_server.webserver.ApiServer.start_api", MagicMock())
    apiserver = ApiServer(default_conf)
    apiserver.add_rpc_handler(RPC(get_patched_HuangTraderbot(mocker, default_conf)))
...
def test_api_UvicornServer(mocker):
    thread_mock = mocker.patch("HuangTrader.rpc.api_server.uvicorn_threaded.threading.Thread")
...
def test_api_UvicornServer_run(mocker):
    serve_mock = mocker.patch(
        "HuangTrader.rpc.api_server.uvicorn_threaded.UvicornServer.serve", get_mock_coro(None)
    )
    s = UvicornServer(uvicorn.Config(MagicMock(), port=8080, host="127.0.0.1"))
...
def test_api_run(default_conf, mocker, caplog):
    ...
    mocker.patch("HuangTrader.rpc.api_server.webserver.UvicornServer", server_mock)
    ...
    caplog.clear()
    mocker.patch(
        "HuangTrader.rpc.api_server.webserver.UvicornServer", MagicMock(side_effect=Exception)
    )
    apiserver.start_api()
...
def test_api_show_config(botclient):
    ...
    assert response["bot_name"] == "HuangTrader"
...
def test_api_pair_history(botclient, tmp_path, mocker):
    ...
    timeframe = "5m"
    lfm = mocker.patch("HuangTrader.strategy.interface.IStrategy.load_freqAI_model")
    # No pair
    rc = client_get(
        client,
...
def test_api_plot_config(botclient, mocker, tmp_path):
    ...
    mocker.patch("HuangTrader.rpc.api_server.api_v1.get_rpc_optional", return_value=None)
...
def test_api_strategy(botclient, tmp_path, mocker):
    ...
    mocker.patch(
        "HuangTrader.resolvers.strategy_resolver.StrategyResolver._load_strategy",
...
def test_api_freqaimodels(botclient, tmp_path, mocker):
    ...
    ftbot.config["user_data_dir"] = tmp_path
    mocker.patch(
        "HuangTrader.resolvers.freqaimodel_resolver.FreqaiModelResolver.search_all_objects",
        return_value=[
...
def test_api_pairlists_evaluate(botclient, tmp_path, mocker):
    ...
    plm = mocker.patch(
        "HuangTrader.rpc.api_server.api_background_tasks.__run_pairlist", return_value=None
    )
    body = {
        "pairlists": [
            {
...
def test_api_backtesting(botclient, mocker, fee, caplog, tmp_path):
    try:
        ...
        mocker.patch(
            "HuangTrader.optimize.backtesting.Backtesting.backtest_one_strategy",
...
def test_api_backtest_history(botclient, mocker, testdatadir):
    ftbot, client = botclient
    mocker.patch(
        "HuangTrader.data.btanalysis._get_backtest_files",
        return_value=[
...
def test_api_ws_subscribe(botclient, mocker):
    ...
    sub_mock = mocker.patch("HuangTrader.rpc.api_server.ws.WebSocketChannel.set_subscriptions")
...
def test_api_ws_send_msg(default_conf, mocker, caplog):
    try:
        ...
        mocker.patch("HuangTrader.rpc.telegram.Telegram._init")
        mocker.patch("HuangTrader.rpc.api_server.ApiServer.start_api")
        apiserver = ApiServer(default_conf)
        apiserver.add_rpc_handler(RPC(get_patched_HuangTraderbot(mocker, default_conf)))
