...
from HuangTrader.enums import RPCMessageType
from HuangTrader.rpc import RPCManager
from HuangTrader.rpc.api_server.webserver import ApiServer
from tests.conftest import get_patched_HuangTraderbot, log_has


def test__init__(mocker, default_conf) -> None:
    default_conf["telegram"]["enabled"] = False

    rpc_manager = RPCManager(get_patched_HuangTraderbot(mocker, default_conf))
...
def test_cleanup_telegram_disabled(mocker, default_conf, caplog) -> None:
    caplog.set_level(logging.DEBUG)
    telegram_mock = mocker.patch("HuangTrader.rpc.telegram.Telegram.cleanup", MagicMock())
    default_conf["telegram"]["enabled"] = False

    HuangTraderbot = get_patched_HuangTraderbot(mocker, default_conf)
    rpc_manager = RPCManager(HuangTraderbot)
    rpc_manager.cleanup()
...
def test_send_msg_telegram_disabled(mocker, default_conf, caplog) -> None:
    telegram_mock = mocker.patch("HuangTrader.rpc.telegram.Telegram.send_msg", MagicMock())
    default_conf["telegram"]["enabled"] = False
...
def test_send_msg_telegram_error(mocker, default_conf, caplog) -> None:
    mocker.patch("HuangTrader.rpc.telegram.Telegram._init", MagicMock())
    mocker.patch("HuangTrader.rpc.telegram.Telegram.send_msg", side_effect=ValueError())
    default_conf["telegram"]["enabled"] = True
    HuangTraderbot = get_patched_HuangTraderbot(mocker, default_conf)
    rpc_manager = RPCManager(HuangTraderbot)
    rpc_manager.send_msg({"type": RPCMessageType.STATUS, "status": "test"})
...
def test_process_msg_queue(mocker, default_conf, caplog) -> None:
    telegram_mock = mocker.patch("HuangTrader.rpc.telegram.Telegram.send_msg")
    default_conf["telegram"]["enabled"] = True
...
def test_send_msg_webhook_CustomMessagetype(mocker, default_conf, caplog) -> None:
    ...
    default_conf["webhook"] = {"enabled": True, "url": "https://DEADBEEF.com"}
    mocker.patch(
        "HuangTrader.rpc.webhook.Webhook.send_msg", MagicMock(side_effect=NotImplementedError)
    )
    rpc_manager = RPCManager(get_patched_HuangTraderbot(mocker, default_conf))
...
def test_startupmessages_telegram_enabled(mocker, default_conf) -> None:
    ...
    HuangTraderbot = get_patched_HuangTraderbot(mocker, default_conf)
    rpc_manager = RPCManager(HuangTraderbot)
    rpc_manager.startup_messages(default_conf, HuangTraderbot.pairlists, HuangTraderbot.protections)
...
def test_init_apiserver_disabled(mocker, default_conf, caplog) -> None:
    caplog.set_level(logging.DEBUG)
    run_mock = MagicMock()
    mocker.patch("HuangTrader.rpc.api_server.ApiServer.start_api", run_mock)
    default_conf["telegram"]["enabled"] = False
    rpc_manager = RPCManager(get_patched_HuangTraderbot(mocker, default_conf))
