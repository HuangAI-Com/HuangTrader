...
from HuangTrader.constants import CANCEL_REASON
from HuangTrader.edge import PairInfo
...
from HuangTrader.loggers import setup_logging
from HuangTrader.persistence import PairLocks, Trade
from HuangTrader.persistence.models import Order
from HuangTrader.rpc import RPC
from HuangTrader.rpc.rpc import RPCException
from HuangTrader.rpc.telegram import Telegram, authorized_only
from HuangTrader.util.datetime_helpers import dt_now
...
def mock_exchange_loop(mocker):
    mocker.patch("HuangTrader.exchange.exchange.Exchange._init_async_loop")
...
def get_telegram_testobject(mocker, default_conf, mock=True, ftbot=None):
    ...
    if mock:
        mocker.patch.multiple(
            "HuangTrader.rpc.telegram.Telegram",
            _init=MagicMock(),
            _send_msg=msg_mock,
            _start_thread=MagicMock(),
        )
    if not ftbot:
        ftbot = get_patched_HuangTraderbot(mocker, default_conf)
    rpc = RPC(ftbot)
...
def test_telegram__init__(default_conf, mocker) -> None:
    mocker.patch("HuangTrader.rpc.telegram.Telegram._init", MagicMock())
...
def test_telegram_init(default_conf, mocker, caplog) -> None:
    app_mock = MagicMock()
    mocker.patch("HuangTrader.rpc.telegram.Telegram._start_thread", MagicMock())
    mocker.patch("HuangTrader.rpc.telegram.Telegram._init_telegram_app", return_value=app_mock)
    mocker.patch("HuangTrader.rpc.telegram.Telegram._startup_telegram", AsyncMock())
...
async def test_telegram_startup(default_conf, mocker) -> None:
    ...
    sleep_mock = mocker.patch("HuangTrader.rpc.telegram.asyncio.sleep", AsyncMock())
...
async def test_telegram_cleanup(
    ...
    # mocker.patch('HuangTrader.rpc.telegram.Application', app_mock)
...
async def test_authorized_only(default_conf, mocker, caplog, update) -> None:
    ...
    bot = HuangTraderBot(default_conf)
    rpc = RPC(bot)
...
async def test_telegram_status(default_conf, update, mocker) -> None:
    ...
    mocker.patch("HuangTrader.rpc.telegram.Telegram._status_table", status_table)

    mocker.patch.multiple(
        "HuangTrader.rpc.rpc.RPC",
...
async def test_order_handle(default_conf, update, ticker, fee, mocker) -> None:
    ...
    telegram, HuangTraderbot, msg_mock = get_telegram_testobject(mocker, default_conf)

    patch_get_signal(HuangTraderbot)

    HuangTraderbot.state = State.RUNNING
    msg_mock.reset_mock()

    # Create some test data
    HuangTraderbot.enter_positions()

    mocker.patch("HuangTrader.rpc.telegram.MAX_MESSAGE_LENGTH", 500)
    ...
    msg_mock.reset_mock()
    mocker.patch("HuangTrader.rpc.telegram.MAX_MESSAGE_LENGTH", 50)
    context = MagicMock()
    context.args = ["2"]
    await telegram._order(update=update, context=context)
...
async def test_status_handle(default_conf, update, ticker, fee, mocker) -> None:
    ...
    HuangTraderbot.state = State.STOPPED
...
async def test_daily_handle(default_conf_usdt, update, ticker, fee, mocker, time_machine) -> None:
    mocker.patch("HuangTrader.rpc.rpc.CryptoToFiatConverter._find_price", return_value=1.1)
    mocker.patch.multiple(
        EXMS,
        fetch_ticker=ticker,
        get_fee=fee,
    ...
    telegram, _HuangTraderbot, msg_mock = get_telegram_testobject(mocker, default_conf_usdt)
...
async def test_weekly_handle(default_conf_usdt, update, ticker, fee, mocker, time_machine) -> None:
    ...
    telegram, HuangTraderbot, msg_mock = get_telegram_testobject(mocker, default_conf_usdt)
...
async def test_telegram_profit_handle(
    ...
    mocker.patch("HuangTrader.wallets.Wallets.get_starting_balance", return_value=1000)
    assert (
...
async def test_telegram_stats(default_conf, update, ticker, fee, mocker, is_short) -> None:
    mocker.patch("HuangTrader.rpc.rpc.CryptoToFiatConverter._find_price", return_value=15000.0)
    mocker.patch.multiple(
        EXMS,
        fetch_ticker=ticker,
        get_fee=fee,
...
async def test_balance_handle_empty_response(default_conf, update, mocker) -> None:
    ...
    HuangTraderbot.config["dry_run"] = False
    await telegram._balance(update=update, context=MagicMock())
    result = msg_mock.call_args_list[0][0][0]
    assert msg_mock.call_count == 1
...
async def test_balance_handle_too_large_response(default_conf, update, mocker) -> None:
    ...
    mocker.patch(
        "HuangTrader.rpc.rpc.RPC._rpc_balance",
        return_value={
            "currencies": balances,
            "total": 100.0,
            "total_bot": 100.0,
...
async def test_start_handle(default_conf, update, mocker) -> None:
    ...
    HuangTraderbot.state = State.STOPPED
    assert HuangTraderbot.state == State.STOPPED
    await telegram._start(update=update, context=MagicMock())
    assert HuangTraderbot.state == State.RUNNING
    assert msg_mock.call_count == 1
...
async def test_stopbuy_handle(default_conf, update, mocker) -> None:
    ...
    assert HuangTraderbot.config["max_open_trades"] != 0
...
async def test_reload_config_handle(default_conf, update, mocker) -> None:
    ...
    assert HuangTraderbot.state == State.RELOAD_CONFIG
    assert msg_mock.call_count == 1
...
async def test_telegram_forceexit_handle(
    ...
    mocker.patch("HuangTrader.rpc.rpc.CryptoToFiatConverter._find_price", return_value=15000.0)
    msg_mock = mocker.patch("HuangTrader.rpc.telegram.Telegram.send_msg", MagicMock())
    mocker.patch("HuangTrader.rpc.telegram.Telegram._init", MagicMock())
    patch_exchange(mocker)
    ...
    HuangTraderbot = HuangTraderBot(default_conf)
    rpc = RPC(HuangTraderbot)
    telegram = Telegram(rpc, default_conf)
    patch_get_signal(HuangTraderbot)
...
async def test_telegram_force_exit_down_handle(
    ...
    mocker.patch(
        "HuangTrader.rpc.fiat_convert.CryptoToFiatConverter._find_price", return_value=15000.0
    )
    msg_mock = mocker.patch("HuangTrader.rpc.telegram.Telegram.send_msg", MagicMock())
    mocker.patch("HuangTrader.rpc.telegram.Telegram._init", MagicMock())
    patch_exchange(mocker)
...
async def test_force_exit_no_pair(default_conf, update, ticker, fee, mocker) -> None:
    ...
    femock = mocker.patch("HuangTrader.rpc.rpc.RPC._rpc_force_exit")
    telegram, HuangTraderbot, msg_mock = get_telegram_testobject(mocker, default_conf)
...
async def test_force_enter_handle(default_conf, update, mocker) -> None:
    ...
    mocker.patch("HuangTrader.rpc.rpc.RPC._rpc_force_entry", fbuy_mock)

    telegram, HuangTraderbot, _ = get_telegram_testobject(mocker, default_conf)
    patch_get_signal(HuangTraderbot)
...
async def test_telegram_entry_tag_performance_handle(
    ...
    msg_mock.reset_mock()
    mocker.patch(
        "HuangTrader.rpc.rpc.RPC._rpc_enter_tag_performance", side_effect=RPCException("Error")
    )
...
async def test_telegram_exit_reason_performance_handle(
    ...
    msg_mock.reset_mock()
    mocker.patch(
        "HuangTrader.rpc.rpc.RPC._rpc_exit_reason_performance", side_effect=RPCException("Error")
    )
...
async def test_telegram_mix_tag_performance_handle(
    ...
    msg_mock.reset_mock()
    mocker.patch(
        "HuangTrader.rpc.rpc.RPC._rpc_mix_tag_performance", side_effect=RPCException("Error")
    )
...
async def test_whitelist_static(default_conf, update, mocker) -> None:
    telegram, _HuangTraderbot, msg_mock = get_telegram_testobject(mocker, default_conf)
...
async def test_blacklist_static(default_conf, update, mocker) -> None:
    ...
    assert HuangTraderbot.pairlists.blacklist == ["DOGE/BTC", "HOT/BTC", "ETH/BTC"]
    ...
    assert HuangTraderbot.pairlists.blacklist == ["DOGE/BTC", "HOT/BTC", "ETH/BTC", "XRP/.*"]
...
async def test_telegram_logs(default_conf, update, mocker) -> None:
    ...
    assert msg_mock.call_count == 1
    assert "HuangTrader\\.rpc\\.telegram" in msg_mock.call_args_list[0][0][0]
    ...
    mocker.patch("HuangTrader.rpc.telegram.MAX_MESSAGE_LENGTH", 200)
    context = MagicMock()
    context.args = []
    await telegram._logs(update=update, context=context)
...
async def test_edge_enabled(edge_conf, update, mocker) -> None:
    mocker.patch(
        "HuangTrader.edge.Edge._cached_pairs",
        mocker.PropertyMock(
            return_value={
    ...
    mocker.patch("HuangTrader.edge.Edge._cached_pairs", mocker.PropertyMock(return_value={}))
    await telegram._edge(update=update, context=MagicMock())
    assert msg_mock.call_count == 1
...
async def test_version_handle(default_conf, update, mocker) -> None:
    ...
    msg_mock.reset_mock()
    HuangTraderbot.strategy.version = lambda: "1.1.1"
...
async def test_show_config_handle(default_conf, update, mocker) -> None:
    ...
    msg_mock.reset_mock()
    HuangTraderbot.config["trailing_stop"] = True
    await telegram._show_config(update=update, context=MagicMock())
    assert msg_mock.call_count == 1
    assert "*Mode:* `{}`".format("Dry-run") in msg_mock.call_args_list[0][0][0]
    assert "*Exchange:* `binance`" in msg_mock.call_args_list[0][0][0]
...
def test_send_msg_enter_notification(
    ...
    mocker.patch(
        "HuangTrader.data.dataprovider.DataProvider.get_analyzed_dataframe", return_value=(df, 1)
    )
    ...
    HuangTraderbot.config["telegram"]["notification_settings"] = {"entry": "off"}
    caplog.clear()
    msg_mock.reset_mock()
    telegram.send_msg(msg)
    ...
    HuangTraderbot.config["telegram"]["notification_settings"] = {"entry": "silent"}
    caplog.clear()
    msg_mock.reset_mock()
...
async def test__send_msg_keyboard(default_conf, mocker, caplog) -> None:
    ...
    bot = MagicMock()
    bot.send_message = AsyncMock()
    HuangTraderbot = get_patched_HuangTraderbot(mocker, default_conf)
    rpc = RPC(HuangTraderbot)
    ...
    def init_telegram(HuangTraderbot):
        telegram = Telegram(rpc, default_conf)
        telegram._app = MagicMock()
        telegram._app.bot = bot
        return telegram
    ...
    HuangTraderbot.config["telegram"]["enabled"] = True
    telegram = init_telegram(HuangTraderbot)
    await telegram._send_msg("test")
    ...
    HuangTraderbot.config["telegram"]["enabled"] = True
    HuangTraderbot.config["telegram"]["keyboard"] = invalid_keys_list
    err_msg = (
        re.escape(
    ...
    HuangTraderbot.config["telegram"]["enabled"] = True
    HuangTraderbot.config["telegram"]["keyboard"] = custom_keys_list
    telegram = init_telegram(HuangTraderbot)
    await telegram._send_msg("test")
    used_keyboard = bot.send_message.call_args[1]["reply_markup"]
...
async def test_change_market_direction(default_conf, mocker, update) -> None:
    ...
    assert telegram._rpc._HuangTrader.strategy.market_direction == MarketDirection.NONE
    context = MagicMock()
    context.args = ["long"]
    await telegram._changemarketdir(update, context)
    assert telegram._rpc._HuangTrader.strategy.market_direction == MarketDirection.LONG
    context = MagicMock()
