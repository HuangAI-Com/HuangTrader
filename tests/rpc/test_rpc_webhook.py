...
from HuangTrader.enums import ExitType, RPCMessageType
from HuangTrader.rpc import RPC
from HuangTrader.rpc.discord import Discord
from HuangTrader.rpc.webhook import Webhook
...
def get_webhook_dict() -> dict:
    return {
        "enabled": True,
        "url": "https://maker.ifttt.com/trigger/HuangTrader_test/with/key/c764udvJ5jfSlswVRukZZ2/",
        "webhookentry": {
...
def test__init__(mocker, default_conf):
    ...
    webhook = Webhook(RPC(get_patched_HuangTraderbot(mocker, default_conf)), default_conf)
...
def test_send_msg_webhook(default_conf, mocker):
    ...
    msg_mock = MagicMock()
    mocker.patch("HuangTrader.rpc.webhook.Webhook._send_msg", msg_mock)
    webhook = Webhook(RPC(get_patched_HuangTraderbot(mocker, default_conf)), default_conf)
    # Test buy
    msg_mock = MagicMock()
    mocker.patch("HuangTrader.rpc.webhook.Webhook._send_msg", msg_mock)
...
def test__send_msg(default_conf, mocker, caplog):
    default_conf["webhook"] = get_webhook_dict()
    webhook = Webhook(RPC(get_patched_HuangTraderbot(mocker, default_conf)), default_conf)
    msg = {"value1": "DEADBEEF", "value2": "ALIVEBEEF", "value3": "HuangTrader"}
    post = MagicMock()
    mocker.patch("HuangTrader.rpc.webhook.post", post)
...
def test_send_msg_discord(default_conf, mocker):
    ...
    msg_mock = MagicMock()
    mocker.patch("HuangTrader.rpc.webhook.Webhook._send_msg", msg_mock)
    discord = Discord(RPC(get_patched_HuangTraderbot(mocker, default_conf)), default_conf)
