...
from HuangTrader.constants import CUSTOM_TAG_MAX_LENGTH
from HuangTrader.data.dataprovider import DataProvider
from HuangTrader.data.history import load_data
from HuangTrader.enums import ExitCheckTuple, ExitType, HyperoptState, SignalDirection
from HuangTrader.exceptions import OperationalException, StrategyError
from HuangTrader.optimize.hyperopt_tools import HyperoptStateContainer
...
from HuangTrader.strategy.hyper import detect_parameters
...
def test_advise_all_indicators_copy(mocker, default_conf, testdatadir) -> None:
    strategy = StrategyResolver.load_strategy(default_conf)
    aimock = mocker.patch("HuangTrader.strategy.interface.IStrategy.advise_indicators")
    timerange = TimeRange.parse_timerange("1510694220-1510700340")
    data = load_data(testdatadir, "1m", ["UNITTEST/BTC"], timerange=timerange, fill_up_missing=True)
...
def test_auto_hyperopt_interface_loadparams(default_conf, mocker, caplog):
    ...
    mocker.patch("HuangTrader.strategy.hyper.HyperoptTools.load_params", return_value=expected_result)
    PairLocks.timeframe = default_conf["timeframe"]
    strategy = StrategyResolver.load_strategy(default_conf)
    ...
    mocker.patch(
        "HuangTrader.strategy.hyper.HyperoptTools.load_params", MagicMock(side_effect=ValueError())
    )
