...
from HuangTrader.configuration import Configuration, validate_config_consistency
from HuangTrader.configuration.config_secrets import sanitize_config
from HuangTrader.configuration.config_validation import validate_config_schema
from HuangTrader.configuration.deprecated_settings import (
    check_conflicting_settings,
    process_deprecated_setting,
    process_removed_setting,
    process_temporary_deprecated_settings,
)
from HuangTrader.configuration.environment_vars import _flat_vars_to_nested_dict
from HuangTrader.configuration.load_config import (
    load_config_file,
    load_file,
    load_from_files,
    log_config_error_range,
)
from HuangTrader.constants import DEFAULT_DB_DRYRUN_URL, DEFAULT_DB_PROD_URL, ENV_VAR_PREFIX
...
def test_load_config_file(default_conf, mocker, caplog) -> None:
    ...
    file_mock = mocker.patch(
        "HuangTrader.configuration.load_config.Path.open",
...
def test_load_config_file_error(default_conf, mocker, caplog) -> None:
    ...
    mocker.patch(
        "HuangTrader.configuration.load_config.Path.open", mocker.mock_open(read_data=filedata)
    )
...
def test_load_config_combine_dicts(default_conf, mocker, caplog) -> None:
    ...
    mocker.patch("HuangTrader.configuration.load_config.load_config_file", configsmock)
...
def test_print_config(default_conf, mocker, caplog) -> None:
    ...
    configsmock = MagicMock(side_effect=config_files)
    mocker.patch("HuangTrader.configuration.configuration.create_datadir", lambda c, x: x)
    mocker.patch("HuangTrader.configuration.configuration.load_from_files", configsmock)
...
def test_load_config_file_exception(mocker) -> None:
    mocker.patch(
        "HuangTrader.configuration.configuration.Path.open",
...
def test_setup_configuration_with_arguments(mocker, default_conf, caplog, tmp_path) -> None:
    ...
    mocker.patch("HuangTrader.configuration.configuration.create_datadir", lambda c, x: x)
    mocker.patch(
        "HuangTrader.configuration.configuration.create_userdata_dir",
    ...
    assert log_has(f"Using user-data directory: {tmp_path / 'HuangTrader'} ...", caplog)
    assert "user_data_dir" in config
...
def test_cli_verbose_with_params(default_conf, mocker, caplog) -> None:
    ...
    mocker.patch("HuangTrader.loggers.set_loggers", MagicMock)
    arglist = ["trade", "-vvv"]
    args = Arguments(arglist).get_parsed_arg()
...
def test_pairlist_resolving_fallback(mocker, tmp_path):
    ...
    mocker.patch(
        "HuangTrader.configuration.configuration.load_file",
...
def test_flat_vars_to_nested_dict(caplog):
    test_args = {
        "HuangTrader__EXCHANGE__SOME_SETTING": "true",
        "HuangTrader__EXCHANGE__SOME_FALSE_SETTING": "false",
        "HuangTrader__EXCHANGE__CONFIG__whatever": "sometime",
        "HuangTrader__EXIT_PRICING__PRICE_SIDE": "bid",
        "HuangTrader__EXIT_PRICING__cccc": "500",
        "HuangTrader__STAKE_AMOUNT": "200.05",
        "HuangTrader__TELEGRAM__CHAT_ID": "2151",
    ...
    assert log_has("Loading variable 'HuangTrader__EXCHANGE__SOME_SETTING'", caplog)
