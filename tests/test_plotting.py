...
from HuangTrader.commands import start_plot_dataframe, start_plot_profit
from HuangTrader.configuration import TimeRange
from HuangTrader.data import history
from HuangTrader.data.btanalysis import load_backtest_data
from HuangTrader.data.metrics import create_cum_profit
from HuangTrader.exceptions import OperationalException
from HuangTrader.plot.plotting import (
    add_areas,
    add_indicators,
    add_profit,
    create_plotconfig,
...
def test_generate_candlestick_graph_no_signals_no_trades(default_conf, mocker, testdatadir, caplog):
    row_mock = mocker.patch(
        "HuangTrader.plot.plotting.add_indicators", MagicMock(side_effect=fig_generating_mock)
    )
    trades_mock = mocker.patch(
        "HuangTrader.plot.plotting.plot_trades", MagicMock(side_effect=fig_generating_mock)
    )
...
def test_generate_Plot_filename():
    ...
    assert fn == "HuangTrader-plot-UNITTEST_BTC-5m.html"


def test_generate_plot_file(mocker, caplog, user_dir):
    fig = generate_empty_figure()
    plot_mock = mocker.patch("HuangTrader.plot.plotting.plot", MagicMock())
    store_plot_file(
        fig, filename="HuangTrader-plot-UNITTEST_BTC-5m.html", directory=user_dir / "plot"
    )

    expected_fn = str(user_dir / "plot/HuangTrader-plot-UNITTEST_BTC-5m.html")
...
def test_generate_profit_graph(testdatadir):
    ...
    assert fig.layout.title.text == "HuangTrader Profit plot"
...
def test_start_plot_dataframe(mocker):
    aup = mocker.patch("HuangTrader.plot.plotting.load_and_plot_trades", MagicMock())
    args = [
        "plot-dataframe",
        "--config",
...
def test_start_plot_profit(mocker):
    aup = mocker.patch("HuangTrader.plot.plotting.plot_profit", MagicMock())
    args = [
        "plot-profit",
        "--config",
        "tests/testdata/testconfigs/main_test_config.json",
...
def test_plot_profit(default_conf, mocker, testdatadir):
    ...
    store_mock = MagicMock()
    mocker.patch.multiple(
        "HuangTrader.plot.plotting", generate_profit_graph=profit_mock, store_plot_file=store_mock
    )
    with pytest.raises(
